...
public class BenefitsCalculationLookupableHelperServiceImpl extends AbstractLookupableHelperServiceImpl {
    ...
    public List<Column> getColumns() {
        ...
        if(offsetParmValue.equalsIgnoreCase("n")) {
            for(Iterator<Column> it = columns.iterator(); it.hasNext(); ) {
                ...
                if(column.getPropertyName().equalsIgnoreCase(LdConstants.ACCOUNT_CODE_OFFSET_PROPERTY_NAME) || column.getPropertyName().equalsIgnoreCase(LdConstants.OBJECT_CODE_OFFSET_PROPERTY_NAME)) {
                    it.remove();
                }
            }
        }
        ...
        if(offsetParmValue.equalsIgnoreCase("n")) {
            for(Iterator<Row> it = rows.iterator(); it.hasNext(); ) {
                ...
                for(Field field : row.getFields()) {
                    if(field.getPropertyName().equalsIgnoreCase(LdConstants.ACCOUNT_CODE_OFFSET_PROPERTY_NAME) || field.getPropertyName().equalsIgnoreCase(LdConstants.OBJECT_CODE_OFFSET_PROPERTY_NAME)) {
                        it.remove();
                    }
                }
            }
    ...
    public void validateSearchParameters(Map fieldValues) {
        ...
        if (StringUtils.isNotEmpty(accountNumber)) {
            fieldsMap.clear();
            fieldsMap.put(Constant.ACCT_NUMBER, accountNumber);
        ...
        if (StringUtils.isNotEmpty(objectCode)) {
            fieldsMap.clear();
            fieldsMap.put(Constant.FIN_OBJECT_CD, objectCode);
