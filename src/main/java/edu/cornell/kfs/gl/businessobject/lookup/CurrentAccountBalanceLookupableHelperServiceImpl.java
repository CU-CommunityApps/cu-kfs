...
public class CurrentAccountBalanceLookupableHelperServiceImpl extends AbstractGeneralLedgerLookupableHelperServiceImpl {
	...
	Collection<String> cashBudgetRecordLevelCodes = this.getParameterService().getParameterValuesAsString(CurrentAccountBalance.class,
		CUKFSParameterKeyConstants.GeneralLedgerSysParmeterKeys.CASH_BUDGET_RECORD_LEVEL_PARM);
	Collection<String> expenseObjectTypeCodes = this.getParameterService().getParameterValuesAsString(CurrentAccountBalance.class,
		CUKFSParameterKeyConstants.GeneralLedgerSysParmeterKeys.EXPENSE_OBJECT_TYPE_CODE_PARAM);
	Collection<String> fundBalanceObjCodes = this.getParameterService().getParameterValuesAsString(CurrentAccountBalance.class,
		CUKFSParameterKeyConstants.GeneralLedgerSysParmeterKeys.FUND_BALANCE_OBJECT_CODE_PARAM);
	Collection<String> currentAssetObjTypeCodes = this.getParameterService().getParameterValuesAsString(CurrentAccountBalance.class,
		CUKFSParameterKeyConstants.GeneralLedgerSysParmeterKeys.CURRENT_ASSET_OBJECT_CODE_PARAM);
	Collection<String> currentLiabilityObjTypeCodes = this.getParameterService().getParameterValuesAsString(CurrentAccountBalance.class,
		CUKFSParameterKeyConstants.GeneralLedgerSysParmeterKeys.CURRENT_LIABILITY_OBJECT_CODE_PARAM);
        Collection<String> incomeObjTypeCodes = this.getParameterService().getParameterValuesAsString(CurrentAccountBalance.class,
		CUKFSParameterKeyConstants.GeneralLedgerSysParmeterKeys.INCOME_OBJECT_TYPE_CODE_PARAM);
	Collection<String> encumbranceBalTypes = this.getParameterService().getParameterValuesAsString(CurrentAccountBalance.class,
		CUKFSParameterKeyConstants.GeneralLedgerSysParmeterKeys.ENCUMBRANCE_BALANCE_TYPE_PARAM);
	boolean cbExclusion = this.getParameterService().getParameterValueAsBoolean(CurrentAccountBalance.class, 
			CUKFSParameterKeyConstants.GeneralLedgerSysParmeterKeys.EXCLUDE_CB_PERIOD_PARAM);
	...
	if (StringUtils.isBlank(accountNumber) && StringUtils.isBlank(organizationCode) && StringUtils.isBlank(fiscalOfficerPrincipalName) && StringUtils.isBlank(supervisorPrincipalName)) {
	    GlobalVariables.getMessageMap().putError(KFSPropertyConstants.ACCOUNT_NUMBER, CUKFSKeyConstants.ERROR_CURRBALANCE_LOOKUP_CRITERIA_REQD);
	    throw new ValidationException("errors in search criteria");
	}
