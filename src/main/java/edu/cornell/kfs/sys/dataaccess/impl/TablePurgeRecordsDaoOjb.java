...
public abstract class TablePurgeRecordsDaoOjb extends PlatformAwareDaoBaseOjb implements TablePurgeRecordsDao {
    ...
    protected abstract Criteria buildRecordDeletionCriteria(Object recordIdentifiedForDeletion);
    
    protected abstract String buildPurgeRecordingString(Object recordIdentifiedForDeletion);
    ...
    protected void identifyAndRequestRecordsDeletion(Class classForDeleteQuery, Criteria lookupCriteria) {
        ...
        for (Object recordAsObject : toBePurgedRecords) {
            deleteRecordBasedOnCriteria(classForDeleteQuery, buildRecordDeletionCriteria(recordAsObject), buildPurgeRecordingString(recordAsObject));
        }
    }
    
    @Transactional
    protected void deleteRecordBasedOnCriteria(Class classForDeleteQuery, Criteria deletionCriteria, String purgeRecordingString) {
        try {
            getPersistenceBrokerTemplate().deleteByQuery(QueryFactory.newQuery(classForDeleteQuery, deletionCriteria));
            LOG.info("deleteRecordBasedOnCriteria: Purged record :: " + purgeRecordingString);
        } catch (DataAccessException dae) {
            LOG.info("deleteRecordBasedOnCriteria: DataAccessException caught attempting to delete/purge database table record: " + purgeRecordingString);
        }
    }
    
    protected Date getJavaSqlDateForComputedDate(java.util.Date jobRunDate, int daysOld) {
