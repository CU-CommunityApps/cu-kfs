...
public class CuSubAccountPreRules extends SubAccountPreRules {
  ...
  protected boolean doCustomPreRules(final MaintenanceDocument document) {
    ...
    @SuppressWarnings("deprecation")
    protected void copyICRFromAccount(final MaintenanceDocument document) {
        ...
        final MaintenanceDocumentRestrictions auths = SpringContext.getBean(BusinessObjectAuthorizationService.class)
        ...
        if (!auths.getFieldRestriction("a21SubAccount.financialIcrSeriesIdentifier").isReadOnly()) {
            ...
            final A21SubAccount a21SubAccount = newSubAccount.getA21SubAccount();
            ...
            if (KFSConstants.SubAccountType.EXPENSE.equals(a21SubAccount.getSubAccountTypeCode())) {
                ...
                if (ObjectUtils.isNotNull(account)) {
                    if (a21SubAccount.getA21ActiveIndirectCostRecoveryAccounts().isEmpty()) {
                        for (final IndirectCostRecoveryAccount icrAccount : account.getActiveIndirectCostRecoveryAccounts()){
    ...
    protected boolean checkOffCampus(final MaintenanceDocument saccDoc) {
      ...
      final boolean saccOffCampus = newSubAccount.getA21SubAccount().getOffCampusCode();

      if (saccOffCampus) {
        final String questionText = SpringContext.getBean(ConfigurationService.class).getPropertyValueAsString(CUKFSKeyConstants.QUESTION_A21SUBACCOUNT_OFF_CAMPUS_INDICATOR);

        final boolean leaveAsIs = super.askOrAnalyzeYesNoQuestion(CUKFSConstants.A21SubAccountDocumentConstants.OFF_CAMPUS_INDICATOR_QUESTION_ID, questionText);
