               ...
               // }
                ...
                if(ObjectUtils.isNotNull(indirectCostRecoveryAccounts) && indirectCostRecoveryAccounts.size()>0){
                	for(IndirectCostRecoveryAccountChange icrAcctChange : indirectCostRecoveryAccounts){
                		...
                		if(ObjectUtils.isNotNull(account.getIndirectCostRecoveryAccounts()) && account.getIndirectCostRecoveryAccounts().size()>0){
                			for(IndirectCostRecoveryAccount existingIcrAcct : account.getIndirectCostRecoveryAccounts()){
                				if(existingIcrAcct.getChartOfAccountsCode().equalsIgnoreCase(icrAcctChange.getIndirectCostRecoveryFinCoaCode()) && existingIcrAcct.getIndirectCostRecoveryAccountNumber().equalsIgnoreCase(icrAcctChange.getIndirectCostRecoveryAccountNumber())){
                					existingIcrAcct.setAccountLinePercent(icrAcctChange.getAccountLinePercent());
                					existingIcrAcct.setActive(icrAcctChange.isActive());
                					update = true;
                				}
                			}	
                		}
