...
public class SecurityRequestDocumentServiceImpl implements SecurityRequestDocumentService {
            ...
            if (groupTab.isActive()) {
                for (SecurityProvisioningGroup provisioningGroup : groupTab.getSecurityProvisioningGroups()) {
                    if (provisioningGroup.isActive()) {
                        ...
                        requestRole.setAllowKSRToManageQualifications(!isKSRPreventedFromEditingRoleQualificationsForRole(provisioningGroup.getRoleId()));
    ...
    protected boolean isKSRPreventedFromEditingRoleQualificationsForRole(String roleId) {
        ...
        if (roleIdsPreventQualificationEdit.contains(roleId)) {
            LOG.debug("isKSRPreventedFromEditingRoleQualificationsForRole, role ID {} is in parameter returning true", roleId);
            return true;
        } else {
            LOG.debug("isKSRPreventedFromEditingRoleQualificationsForRole, role ID {} is NOT in parameter returning false", roleId);
            return false;
        }
    }
