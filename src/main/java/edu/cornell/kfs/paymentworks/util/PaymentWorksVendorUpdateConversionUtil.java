...
public class PaymentWorksVendorUpdateConversionUtil {
	...
	public PaymentWorksVendor createPaymentWorksVendorUpdate(
		...
		// remittance address
		if (StringUtils.equals(paymentWorksVendorUpdatesDTO.getGroup_name(),
		...
		if (ObjectUtils.isNotNull(paymentWorksVendorUpdatesDTO.getVendor_nums().getVendorNumbers())
			...
			for (PaymentWorksVendorNumberDTO vendorNumber : paymentWorksVendorUpdatesDTO.getVendor_nums()
					.getVendorNumbers()) {
			...
			if (vendorNumberStr.length() > 0) {
				paymentWorksVendor
						.setVendorNumberList(vendorNumberStr.toString().substring(0, vendorNumberStr.length() - 1));
			}
		}
	...
	public boolean duplicateFieldsOnVendor(VendorDetail vendorDetail, PaymentWorksVendor paymentWorksVendor) {
		...
		if (StringUtils.equals(paymentWorksVendor.getGroupName(), PaymentWorksConstants.VendorUpdateGroups.COMPANY)) {
			...
			if (existingVendorAlias(vendorDetail, paymentWorksVendor)
					&& isEqualsIgnoreNull(paymentWorksVendor.getRequestingCompanyTin(),
							vendorDetail.getVendorHeader().getVendorTaxNumber())
					&& existingVendorPhoneNumber(vendorDetail, paymentWorksVendor)
					&& isEqualsIgnoreNull(paymentWorksVendor.getRequestingCompanyLegalName(),
							vendorDetail.getVendorName())
					&& isEqualsIgnoreNull(paymentWorksVendor.getRequestingCompanyDuns(),
							vendorDetail.getVendorDunsNumber())
					...
					&& isEqualsIgnoreNull(paymentWorksVendor.getRequestingCompanyUrl(),
							vendorDetail.getVendorUrlAddress())
					&& isEqualsIgnoreNull(paymentWorksVendor.getRequestingCompanyTaxCountry(),
		...
		if (StringUtils.equals(paymentWorksVendor.getGroupName(), PaymentWorksConstants.VendorUpdateGroups.REMIT_ADDRESS)) {
			...
			if (ObjectUtils.isNotNull(address)) {
				...
				if (StringUtils.equalsIgnoreCase(StringUtils.defaultString(
						paymentWorksVendor.getRemittanceAddressCountry(), address.getVendorCountryCode()), "US")) {
					stateMatch = isEqualsIgnoreNull(paymentWorksVendor.getRemittanceAddressState(),
							address.getVendorStateCode());
				} else {
					stateMatch = isEqualsIgnoreNull(paymentWorksVendor.getRemittanceAddressState(),
							address.getVendorAddressInternationalProvinceName());
				}

				if (isEqualsIgnoreNull(paymentWorksVendor.getRemittanceAddressStreet1(),
						address.getVendorLine1Address())
						&& isEqualsIgnoreNull(paymentWorksVendor.getRemittanceAddressStreet2(),
								address.getVendorLine2Address())
						&& isEqualsIgnoreNull(paymentWorksVendor.getRemittanceAddressCity(),
								address.getVendorCityName())
						&& isEqualsIgnoreNull(paymentWorksVendor.getRemittanceAddressCountry(),
								address.getVendorCountryCode())
						&& isEqualsIgnoreNull(paymentWorksVendor.getRemittanceAddressZipCode(),
								address.getVendorZipCode())
						&& stateMatch) {
		...
		if (StringUtils.equals(paymentWorksVendor.getGroupName(), PaymentWorksConstants.VendorUpdateGroups.CORP_ADDRESS)) {
			...
			if (ObjectUtils.isNotNull(address)) {
				...
				if (StringUtils.isNotBlank(paymentWorksVendor.getCorpAddressState())) {
					if (StringUtils.equalsIgnoreCase("EN",
							vendorDetail.getVendorHeader().getVendorOwnershipCategoryCode())
				...
				if (StringUtils.equalsIgnoreCase(StringUtils.defaultString(paymentWorksVendor.getCorpAddressCountry(),
						address.getVendorCountryCode()), "US")) {
					stateMatch = isEqualsIgnoreNull(paymentWorksVendor.getCorpAddressState(),
							address.getVendorStateCode());
				} else {
					stateMatch = isEqualsIgnoreNull(paymentWorksVendor.getCorpAddressState(),
							address.getVendorAddressInternationalProvinceName());
				...
				if (isEqualsIgnoreNull(paymentWorksVendor.getCorpAddressStreet1(), address.getVendorLine1Address())
						&& isEqualsIgnoreNull(paymentWorksVendor.getCorpAddressStreet2(),
								address.getVendorLine2Address())
						&& isEqualsIgnoreNull(paymentWorksVendor.getCorpAddressCity(), address.getVendorCityName())
						&& isEqualsIgnoreNull(paymentWorksVendor.getCorpAddressCountry(),
								address.getVendorCountryCode())
	...
	public VendorDetail createVendorDetailForEdit(VendorDetail newVendorDetail, VendorDetail oldVendorDetail,
		...
		if (StringUtils.equals(paymentWorksVendor.getGroupName(), PaymentWorksConstants.VendorUpdateGroups.COMPANY)) {
			...
			newVendorDetail.setVendorAliases(
					getVendorAliasesForVendorUpdate(newVendorDetail, oldVendorDetail, paymentWorksVendor));

			newVendorDetail.getVendorHeader()
					.setVendorTaxNumber(StringUtils.defaultIfEmpty(paymentWorksVendor.getRequestingCompanyTin(),
							newVendorDetail.getVendorHeader().getVendorTaxNumber()));

			newVendorDetail.setVendorPhoneNumbers(
					getVendorPhoneNumbersForVendorUpdate(newVendorDetail, oldVendorDetail, paymentWorksVendor));

			newVendorDetail.setVendorName(StringUtils.defaultIfEmpty(paymentWorksVendor.getRequestingCompanyLegalName(),
					newVendorDetail.getVendorName()));
			newVendorDetail.setVendorDunsNumber(StringUtils.defaultIfEmpty(
					paymentWorksVendor.getRequestingCompanyDuns(), newVendorDetail.getVendorDunsNumber()));
			...
			if (StringUtils.isNotBlank(paymentWorksVendor.getRequestingCompanyTaxClassificationCode())) {
				...
				newVendorDetail
						.setTaxableIndicator(isTaxable(paymentWorksVendor.getRequestingCompanyTaxClassificationCode(),
			...
			vendorHeader.setVendorCorpCitizenCode(
					StringUtils.defaultIfEmpty(paymentWorksVendor.getRequestingCompanyTaxCountry(),
			...
			if (StringUtils.isNotBlank(paymentWorksVendor.getRequestingCompanyTaxCountry())) {
				if (!StringUtils.equals(paymentWorksVendor.getRequestingCompanyTaxCountry(), "US")) {
					VendorDetailExtension vde = (VendorDetailExtension) ObjectUtils
							.deepCopy(newVendorDetail.getExtension());
					// vde.setHoldCode("NRA");
		...
		if (StringUtils.equals(paymentWorksVendor.getGroupName(), PaymentWorksConstants.VendorUpdateGroups.REMIT_ADDRESS)) {
			...
			List<VendorAddress> vendorAddresses = (List<VendorAddress>) ObjectUtils
					.deepCopy(new ArrayList<VendorAddress>(newVendorDetail.getVendorAddresses()));
			...
			if (ObjectUtils.isNotNull(address)) {
				...
				for (VendorAddress addr : vendorAddresses) {
					if (addr.getVendorAddressGeneratedIdentifier()
							.equals(address.getVendorAddressGeneratedIdentifier())) {
			...
			if (StringUtils.isNotEmpty(paymentWorksVendor.getRemittanceAddressStreet1())) {
				...
				addressLine2 = StringUtils.defaultIfEmpty(paymentWorksVendor.getRemittanceAddressStreet2(),
						newAddress.getVendorLine2Address());
			}

			newAddress.setVendorLine1Address(StringUtils.defaultIfEmpty(
					paymentWorksVendor.getRemittanceAddressStreet1(), newAddress.getVendorLine1Address()));
			newAddress.setVendorLine2Address(
					StringUtils.defaultIfEmpty(paymentWorksVendor.getRemittanceAddressStreet2(), addressLine2));
			newAddress.setVendorCityName(StringUtils.defaultIfEmpty(paymentWorksVendor.getRemittanceAddressCity(),
					newAddress.getVendorCityName()));
			newAddress.setVendorCountryCode(StringUtils.defaultIfEmpty(paymentWorksVendor.getRemittanceAddressCountry(),
					newAddress.getVendorCountryCode()));

			if (StringUtils.equalsIgnoreCase(newAddress.getVendorCountryCode(), "US")) {
				newAddress.setVendorStateCode(StringUtils.defaultIfEmpty(paymentWorksVendor.getRemittanceAddressState(),
						newAddress.getVendorStateCode()));
				newAddress.setVendorAddressInternationalProvinceName("");
			} else {
				newAddress.setVendorAddressInternationalProvinceName(
						StringUtils.defaultIfEmpty(paymentWorksVendor.getRemittanceAddressState(),
								newAddress.getVendorAddressInternationalProvinceName()));
				newAddress.setVendorStateCode("");
			}

			newAddress.setVendorZipCode(StringUtils.defaultIfEmpty(paymentWorksVendor.getRemittanceAddressZipCode(),
					newAddress.getVendorZipCode()));
		...
		if (StringUtils.equals(paymentWorksVendor.getGroupName(), PaymentWorksConstants.VendorUpdateGroups.CORP_ADDRESS)) {
			...
			newAddress.setVendorLine1Address(StringUtils.defaultIfEmpty(paymentWorksVendor.getCorpAddressStreet1(),
					newAddress.getVendorLine1Address()));
			newAddress.setVendorLine2Address(
					StringUtils.defaultIfEmpty(paymentWorksVendor.getCorpAddressStreet2(), addressLine2));
			newAddress.setVendorCityName(StringUtils.defaultIfEmpty(paymentWorksVendor.getCorpAddressCity(),
					newAddress.getVendorCityName()));
			newAddress.setVendorCountryCode(StringUtils.defaultIfEmpty(paymentWorksVendor.getCorpAddressCountry(),
					newAddress.getVendorCountryCode()));

			if (StringUtils.equalsIgnoreCase(newAddress.getVendorCountryCode(), "US")) {
				newAddress.setVendorStateCode(StringUtils.defaultIfEmpty(paymentWorksVendor.getCorpAddressState(),
						newAddress.getVendorStateCode()));
				newAddress.setVendorAddressInternationalProvinceName("");
			} else {
				newAddress.setVendorAddressInternationalProvinceName(
						StringUtils.defaultIfEmpty(paymentWorksVendor.getCorpAddressState(),
								newAddress.getVendorAddressInternationalProvinceName()));
				newAddress.setVendorStateCode("");
			}

			newAddress.setVendorZipCode(StringUtils.defaultIfEmpty(paymentWorksVendor.getCorpAddressZipCode(),
					newAddress.getVendorZipCode()));
			...
			if (StringUtils.isNotBlank(paymentWorksVendor.getCorpAddressState())) {
				if (StringUtils.equalsIgnoreCase("EN",
						newVendorDetail.getVendorHeader().getVendorOwnershipCategoryCode())
						&& !StringUtils.equalsIgnoreCase("CT", paymentWorksVendor.getCorpAddressState())) {
	 ...
	 * @return
	 */
	protected List<VendorAlias> getVendorAliasesForVendorUpdate(VendorDetail vendorDetail, VendorDetail oldVendorDetail,
			PaymentWorksVendor paymentWorksVendor) {

		List<VendorAlias> aliasList = (List<VendorAlias>) ObjectUtils
				.deepCopy(new ArrayList<VendorAlias>(vendorDetail.getVendorAliases()));
	 ...
	 * @return
	 */
	protected List<VendorPhoneNumber> getVendorPhoneNumbersForVendorUpdate(VendorDetail vendorDetail,
			VendorDetail oldVendorDetail, PaymentWorksVendor paymentWorksVendor) {

		List<VendorPhoneNumber> vendorPhoneNumbers = (List<VendorPhoneNumber>) ObjectUtils
				.deepCopy(new ArrayList<VendorPhoneNumber>(vendorDetail.getVendorPhoneNumbers()));
		boolean existingValueFound = false;
		...
		if (StringUtils.isNotBlank(paymentWorksVendor.getRequestingCompanyTelephone())) {
			...
			for (VendorPhoneNumber phone : vendorPhoneNumbers) {
				if (StringUtils.equals(phone.getVendorPhoneNumber(),
						paymentWorksVendor.getRequestingCompanyTelephoneOldValue())) {
					phone.setActive(false);
				}

				if (StringUtils.equals(phone.getVendorPhoneNumber(),
						paymentWorksVendor.getRequestingCompanyTelephone())) {
	...
	protected boolean isTaxable(String classificationCode, String ownershipCategoryCode) {
		...
		if ((StringUtils.equals(classificationCode, "1") || StringUtils.equals(classificationCode, "2"))
				&& (StringUtils.equals(ownershipCategoryCode, "US") || StringUtils.equals(ownershipCategoryCode, "SE")
						|| StringUtils.isBlank(ownershipCategoryCode))) {
