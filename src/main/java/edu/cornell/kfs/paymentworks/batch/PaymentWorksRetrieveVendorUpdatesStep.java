...
public class PaymentWorksRetrieveVendorUpdatesStep extends AbstractStep {
	...
	private static final org.apache.log4j.Logger LOG = org.apache.log4j.Logger
			.getLogger(PaymentWorksRetrieveVendorUpdatesStep.class);
	...
	protected void processAddressVendorUpdates() {
		...
		List<PaymentWorksVendorUpdatesDTO> results = paymentWorksWebService
				.getPendingAddressVendorUpdatesFromPaymentWorks();
		...
		for (PaymentWorksVendorUpdatesDTO vendorUpdate : results) {
			...
			if (!paymentWorksVendorService.isExistingPaymentWorksVendor(vendorUpdate.getId(),
				...
				try {
					...
					LOG.error(
							"Error processing vendor address update (" + vendorUpdate.getId() + "): " + e.getMessage());
					routed = false;
	...
	protected void processCompanyVendorUpdates() {
		...
		List<PaymentWorksVendorUpdatesDTO> results = paymentWorksWebService
				.getPendingCompanyVendorUpdatesFromPaymentWorks();
		...
		for (PaymentWorksVendorUpdatesDTO vendorUpdate : results) {
			...
			if (!paymentWorksVendorService.isExistingPaymentWorksVendor(vendorUpdate.getId(),
				...
				try {
					...
					LOG.error(
							"Error processing vendor company update (" + vendorUpdate.getId() + "): " + e.getMessage());
					routed = false;
					GlobalVariables.getMessageMap().clearErrorMessages();
				}
			}
	...
	protected void processVendor(PaymentWorksVendor paymentWorksVendor, String requestStatus, String requestStatusText,
			String processStatus, boolean routed, boolean directEdit) {
	...
	protected void updatePaymentWorksVendorStatus(String vendorRequestId, String requestStatus) {
		...
		paymentWorksWebService.updateNewVendorUpdatesStatusInPaymentWorks(updateNewVendorStatusList);
	}
	...
	protected File writePaymentWorksVendorUpdateSummaryReport(VendorUpdateSummary vendorUpdateSummary) {
		...
		if (reportWriterService == null) {
			throw new IllegalStateException(
					"ReportWriterService not configured for PaymentWorks Vendor Update service.");
		} else {
	...
	protected void writeVendorUpdateRejectedSummaryDetailRecords(String subtitle,
			List<VendorUpdateSummaryLine> records) {
