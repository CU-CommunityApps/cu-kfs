...
public class AmazonWebServicesBillingServiceImpl implements AmazonWebServicesBillingService, Serializable {
    ...
    protected boolean processAmazonAccountDetails(List<AmazonAccountDetail> amazonAccountDetails) {
        ...
        for (AmazonAccountDetail amazonAccountDetail : amazonAccountDetails) {
            if (StringUtils.equalsIgnoreCase(CuFPConstants.AmazonWebServiceBillingConstants.INTERNAL_KFS_ACCOUNT_DESCRIPTION, amazonAccountDetail.getKfsAccount())) {
                ...
                didProcessingCompleteWithoutErrors = processExternalBill(amazonAccountDetail) && didProcessingCompleteWithoutErrors;
            }
            count ++;
        }
        LOG.info("Processed " + count + " AWS records");
        return didProcessingCompleteWithoutErrors;
    }

    private boolean processExternalBill(AmazonAccountDetail amazonAccountDetail) {
        boolean valid = true;
        try {
            ...
            if (transactionDTO.isTransactionInputError()) {
                ...
                buidAndRouteDistributionOfIncomeDocument(amazonAccountDetail, transactionDTO);
            }
        } catch (Throwable e) {
            valid = false;
    ...
    private Collection<DocumentHeader> findDocumentHeadersForAmazonDetail(AmazonAccountDetail amazonAccountDetail) {
        ...
        fieldValues.put(CUKFSConstants.DOCUMENT_HEADER_EXPLANATION_FIELD, buildDocumentExplanation(amazonAccountDetail.getAwsAccount()));
        fieldValues.put(CUKFSConstants.DOCUMENT_HEADER_DESCRIPTION_FIELD, buildDocumentDescription(amazonAccountDetail.getBusinessPurpose()));
    ...
    private Note buildDINote() {
        ...
        diNote.setAuthorUniversalIdentifier(GlobalVariables.getUserSession().getLoggedInUserPrincipalName());
