...
public class ScheduledAccountingLineServiceImpl implements ScheduledAccountingLineService {
	...
	public TreeMap<Date, KualiDecimal> generateDatesAndAmounts(ScheduledSourceAccountingLine scheduledAccountingLine, int rowId) {
		...
		while (currentStartDate.before(endDate) && totalAmount.isGreaterThan(KualiDecimal.ZERO)) {
			...
			calendarCalculator = incrementDateByValue(scheduledAccountingLine, calendarCalculator, countToAddToCalendar);
			...
			countToAddToCalendar++;
		}
		boolean showError = Integer.parseInt(scheduledAccountingLine.getPartialTransactionCount()) != countToAddToCalendar;
	...
	private Calendar incrementDateByValue(ScheduledSourceAccountingLine accountingLine, Calendar theCal, int countToAddToCalendar) {
		Calendar incrementedCalendar = (Calendar) theCal.clone();
		incrementedCalendar.add(accountingLine.getScheduleTypeEnum().calendarIncrementorType, countToAddToCalendar);
