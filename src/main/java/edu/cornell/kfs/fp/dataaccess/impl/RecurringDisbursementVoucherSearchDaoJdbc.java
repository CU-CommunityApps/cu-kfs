...
public class RecurringDisbursementVoucherSearchDaoJdbc extends PlatformAwareDaoBaseJdbc implements RecurringDisbursementVoucherSearchDao {
        ...
        return getJdbcTemplate().execute(new ConnectionCallback<Collection<String>>() {
            public Collection<String> doInConnection(Connection con) throws SQLException {
                ...
                try {
                    ...
                    LOG.info("SQL Statement to obtain saved DVs spawned by RCDV for current and past fiscal period follows.");
                    LOG.info("*************************************");
                    LOG.info(selectStatementSql);
                    LOG.info("*************************************");
                    selectStatement = con.prepareStatement(selectStatementSql);
                    queryResultSet = selectStatement.executeQuery();

                    while (queryResultSet.next()) {
                        finalResults.add(queryResultSet.getString(1));
                        LOG.info("Found DV DocID=" +queryResultSet.getString(1)+ "=  DVCheckDateSameAsDueDate=" +queryResultSet.getString(2)+ "=");
                    }
    ...
    private String getSavedDvsSpawnedByRecurringDvForCurrentAndPastFiscalPeriodsSelectSql(Date currentFisalPeriodEndDate) {
        ...
        return new String("SELECT B.DV_DOC_NBR, B.DV_CHECK_DT FROM KFS.FP_DV_DOC_T A, KFS.FP_RCDV_DTL_T B, CYNERGY.KREW_DOC_HDR_T C WHERE A.FDOC_NBR = B.DV_DOC_NBR AND C.DOC_HDR_ID = B.DV_DOC_NBR AND C.DOC_HDR_STAT_CD = '") +KewApiConstants.ROUTE_HEADER_SAVED_CD+ ("' AND B.DV_CHECK_DT <= '" +formatter.format(currentFisalPeriodEndDate)+ "'");
    }
