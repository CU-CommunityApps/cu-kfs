...
public class CemiPaymentElectionDataBuilderCsvImpl extends CemiPaymentElectionDataBuilderBase {

    private final Map<String, CemiCsvWriter> csvWriters;
    private final Map<String, CemiSheetDefinition> sheetDefinitions;

    public CemiPaymentElectionDataBuilderCsvImpl(final CemiOutputDefinition outputDefinition,
            final CemiPaymentElectionDao cemiPaymentElectionDao, AchBankService achBankService,
            DateTimeService dateTimeService, BusinessObjectService businessObjectService,
            final LocalDateTime jobRunDate, final String baseFileDirectory, final boolean maskSensitiveData) throws IOException {
        super(outputDefinition, cemiPaymentElectionDao, achBankService, dateTimeService, businessObjectService, jobRunDate, maskSensitiveData);
    ...
    protected void writeDataToIntermediateStorage(String sheetName, Object rowObject) throws IOException {
        ...
        Validate.validState(sheetDefinition != null, "Unexpected CEMI Payment Election datasheet: %s", sheetName);
        Validate.validState(csvWriter != null, "Unexpected non-writeable Payment Election datasheet: %s", sheetName);
