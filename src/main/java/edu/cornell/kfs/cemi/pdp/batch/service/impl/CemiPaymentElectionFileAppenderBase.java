...
import edu.cornell.kfs.cemi.pdp.batch.service.CemiPaymentElectionFileAppender;
...
public abstract class CemiPaymentElectionFileAppenderBase implements CemiPaymentElectionFileAppender {
    ...
    protected CemiPaymentElectionFileAppenderBase(final CemiOutputDefinition outputDefinition, final LocalDateTime jobRunDate) {
    ...
    @Override
    public void populatePaymentElectionFileFromIntermediateDataStorage(final CemiExcelWriter fileWriter) throws IOException {
        for (final CemiSheetDefinition sheetDefinition : outputDefinition.getSheets()) {
            LOG.info("populatePaymentElectionFileFromIntermediateDataStorage, Writing sheet: {}", sheetDefinition.getName());
