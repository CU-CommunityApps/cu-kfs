...
public abstract class CemiRemitToSupplierFileAppenderBase implements CemiRemitToSupplierFileAppender {
    ...
    protected CemiRemitToSupplierFileAppenderBase(final CemiOutputDefinition outputDefinition, final LocalDateTime jobRunDate) {
    ...
    public void populateRemitToSupplierFileFromIntermediateDataStorage(CemiExcelWriter fileWriter) throws IOException {
        for (final CemiSheetDefinition sheetDefinition : outputDefinition.getSheets()) {
            LOG.info("populateRemitToSupplierFileFromIntermediateDataStorage, Writing sheet: {}", sheetDefinition.getName());
