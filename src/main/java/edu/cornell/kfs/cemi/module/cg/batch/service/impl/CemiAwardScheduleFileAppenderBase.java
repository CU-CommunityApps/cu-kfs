...
import edu.cornell.kfs.cemi.module.cg.batch.service.CemiAwardScheduleFileAppender;
...
public abstract class CemiAwardScheduleFileAppenderBase implements CemiAwardScheduleFileAppender {
    ...
    protected CemiAwardScheduleFileAppenderBase(final CemiOutputDefinition outputDefinition, final LocalDateTime jobRunDate) {
    ...
    @Override
    public void populateAwardScheduleFileFromIntermediateDataStorage(final CemiExcelWriter fileWriter) throws IOException {
        for (final CemiSheetDefinition sheetDefinition : outputDefinition.getSheets()) {
            LOG.info("populateAwardScheduleFileFromIntermediateDataStorage, Writing sheet: {}", sheetDefinition.getName());
