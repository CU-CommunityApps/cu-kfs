 ...
 */
public class TaxProcessing1042DaoJdbc extends TaxProcessingDaoJdbc implements TaxProcessingDao {
	private static final Logger LOG = LogManager.getLogger(TaxProcessing1042DaoJdbc.class);
    ...
    public void doTaxProcessing(String taxType, int reportYear, java.sql.Date startDate, java.sql.Date endDate, boolean vendorForeign, java.util.Date processingStartDate) {
        ...
        TaxDataDefinition taxDataDefinition = taxProcessingService.getDataDefinition(CUTaxKeyConstants.TAX_TABLE_1042S_PREFIX, reportYear);
...
//                TransactionRowPRNCBuilder.For1042S.class
        ...
        EnumMap<TaxStatType, Integer> processingStats = processPayments(summary, reportYear, taxProcessingService);
    ...
    private <T extends TransactionDetailSummary> EnumMap<TaxStatType,Integer> processPayments(Transaction1042SSummary summary, int reportYear, TaxProcessingService taxProcessingService) {
        TaxOutputDefinition taxOutputDefinition = taxProcessingService.getOutputDefinition("tax.format.1042s.", reportYear);
