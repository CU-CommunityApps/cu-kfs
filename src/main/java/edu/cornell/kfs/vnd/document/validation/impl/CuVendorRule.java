	...
	private boolean validateVendorAddressForMethodOfPOTransmission(VendorAddress address, String propertyScope){
		...
		//no-op, nothing to verify, default return value is valid
	    	...
	    	if (valid) {
	    		...
	    		valid &= postalCodeValidationService.validateAddress(address.getVendorCountryCode(), address.getVendorStateCode(), address.getVendorZipCode(), propertyScope + VendorPropertyConstants.VENDOR_ADDRESS_STATE, propertyScope + VendorPropertyConstants.VENDOR_ADDRESS_ZIP);
	    	}        	
