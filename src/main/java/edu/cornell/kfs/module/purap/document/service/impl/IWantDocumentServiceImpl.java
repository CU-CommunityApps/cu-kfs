...
public class IWantDocumentServiceImpl implements IWantDocumentService {
    ...
    */
    public RequisitionDocument setUpRequisitionDetailsFromIWantDoc(IWantDocument iWantDocument,
        ...
        copyIWantDocAttachmentsToReqDoc(requisitionDocument, iWantDocument, requisitionForm, purapService);
    ...
    private void copyIWantDocAttachmentsToReqDoc(RequisitionDocument requisitionDocument, IWantDocument iWantDocument,
        RequisitionForm requisitionForm, PurapService purapService) throws Exception {

        purapService.saveDocumentNoValidation(requisitionDocument);
        if (!CollectionUtils.isEmpty(iWantDocument.getNotes())) {

            for (Note note : iWantDocument.getNotes()) {
                try {
                    ...
                    if (originalAttachment != null) {
                        Attachment newAttachment = SpringContext.getBean(AttachmentService.class).createAttachment(
                        		(PersistableBusinessObject) copyingNote, originalAttachment.getAttachmentFileName(),
                        		originalAttachment.getAttachmentMimeTypeCode(), originalAttachment.getAttachmentFileSize().intValue(),
                        		originalAttachment.getAttachmentContents(), originalAttachment.getAttachmentTypeCode());
