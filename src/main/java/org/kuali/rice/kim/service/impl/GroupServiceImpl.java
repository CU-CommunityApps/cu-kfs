...
public class GroupServiceImpl implements GroupService, GroupUpdateService {
     ...
     */
    // KFSPTS - making person info lookup more efficient
    // this was resulting in a few hundred database connections on user login
    @Transactional
    public Map<String, GroupInfo> getGroupInfos(Collection<String> groupIds) {
        ...
        // get all the groups
        Collection<GroupImpl> groups = getBusinessObjectService().findMatching(GroupImpl.class, Collections.singletonMap(KIMPropertyConstants.Group.GROUP_ID, groupIds));
        
        // get all the attributes and split them by group ID for later processing 
        Collection<GroupAttributeDataImpl> attributes = getBusinessObjectService().findMatching(GroupAttributeDataImpl.class, Collections.singletonMap(KIMPropertyConstants.Group.GROUP_ID, groupIds));
        Map<String,List<GroupAttributeDataImpl>> attrsByGroup = new HashMap<String, List<GroupAttributeDataImpl>>();
        for ( GroupAttributeDataImpl attr : attributes ) {
        	List<GroupAttributeDataImpl> groupAttrs = attrsByGroup.get(attr.getGroupId());
        	if ( groupAttrs == null ) {
        		groupAttrs = new ArrayList<GroupAttributeDataImpl>();
            	attrsByGroup.put(attr.getGroupId(), groupAttrs);            	
        	}
        	groupAttrs.add(attr);
        }
        
        // create the "info" objects from each impl
        for (GroupImpl group : groups) {
            GroupInfo info = new GroupInfo();

            info.setActive(group.isActive());
            info.setGroupDescription(group.getGroupDescription());
            info.setGroupId(group.getGroupId());
            info.setGroupName(group.getGroupName());
            info.setKimTypeId(group.getKimTypeId());
            info.setNamespaceCode(group.getNamespaceCode());
            info.setAttributes(new AttributeSet());
            if ( attrsByGroup.get(group.getGroupId()) != null ) {
	            for ( GroupAttributeDataImpl attr : attrsByGroup.get(group.getGroupId()) ) {
	            	if ( attr.getKimAttribute() != null ) {
	            		info.getAttributes().put(attr.getKimAttribute().getAttributeName(), attr.getAttributeValue());
	            	} else {
	            		info.getAttributes().put("Unknown Attribute ID: " + attr.getKimAttributeId(), attr.getAttributeValue());
	            	}
	            }
            }

            result.put(group.getGroupId(), info);
        }
