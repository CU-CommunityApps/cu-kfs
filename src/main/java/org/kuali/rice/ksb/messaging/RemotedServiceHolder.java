 ...
 * Added a new constructor that allows for explicitly specifying whether
 * or not to copy the given ServiceInfo object. Also modified the
 * ServiceInfo-copying method so that it does not temporarily assign a
 * null value to a field that should already be transient.
 * ====
 * 
...
public class RemotedServiceHolder implements ServiceHolder {
	...
	// ==== CU Customization (KFSPTS-410) ====
	public RemotedServiceHolder(ServiceInfo entry, boolean makeCopy) {
		if (makeCopy) {
	...
	/**
         ...
         */
	// ==== CU Customization (KFSPTS-410): ====
	// Removed the part that sets the service to null, since that field on the ServiceDefinition is transient anyway.
