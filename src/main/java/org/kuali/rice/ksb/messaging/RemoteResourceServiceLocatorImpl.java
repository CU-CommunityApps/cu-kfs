...
/**
 * ====
 * CU Customization (KFSPTS-410):
 * Altered the "getAllServices" method so that, when running in batch mode, an attempt
 * will be made to find the service(s) on the RemotedServiceRegistry if they're not
 * found in this class's map.
 * ====
 */
public class RemoteResourceServiceLocatorImpl extends ResourceLoaderContainer implements Runnable, RemoteResourceServiceLocator {
	...
	// ==== CU Customization (KFSPTS-410) ====
	private Map<QName,MaxAgeSoftReference<List<RemotedServiceHolder>>> tempClients =
			new MaxSizeMap<QName,MaxAgeSoftReference<List<RemotedServiceHolder>>>(50);
	
	private Object tempClientsMutex = new Object();
	...
	public List<RemotedServiceHolder> getAllServices(QName qName) {
		...
		if (clientProxies == null) {
			...
			if (clientProxies == null || clientProxies.size() == 0) {
				// ==== CU Customization (KFSPTS-410) ====
				// If in batch mode, try to find the services in another local temp map or in the RemotedServiceRegistry.
				if (ConfigContext.getCurrentContextConfig().getBatchMode().booleanValue()) {
					// If the services list exists in the map, then return it.
					MaxAgeSoftReference<List<RemotedServiceHolder>> proxiesReference;
					synchronized(tempClientsMutex) {
						proxiesReference = this.tempClients.get(qName);
					}
					if (proxiesReference != null) {
						clientProxies = proxiesReference.get();
						if (clientProxies != null && !clientProxies.isEmpty()) {
							return clientProxies;
						}
					}
					
					// Otherwise, attempt to retrieve the service from the RemotedServiceRegistry.
					ServerSideRemotedServiceHolder tempServiceHolder = KSBServiceLocator.getServiceDeployer().getRemotedServiceHolder(qName);
					if (tempServiceHolder != null) {
						clientProxies = new ArrayList<RemotedServiceHolder>();
						clientProxies.add(new RemotedServiceHolder(tempServiceHolder.getServiceInfo(), true));
						synchronized(tempClientsMutex) {
							this.tempClients.put(qName, new MaxAgeSoftReference<List<RemotedServiceHolder>>(30, clientProxies));
						}
						return clientProxies;
					}
				}
