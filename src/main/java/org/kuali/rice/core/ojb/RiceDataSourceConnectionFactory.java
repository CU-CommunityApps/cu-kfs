...
public class RiceDataSourceConnectionFactory extends ConnectionFactoryNotPooledImpl {
	...
	public Connection lookupConnection(JdbcConnectionDescriptor jcd) throws LookupException {
		if ( jcd == null ) {
			throw new LookupException( "ERROR: null JdbcConnectionDescriptor passed into lookupConnection()" );
		}
		try {
			// KFSPTS-335 - fix for over-synchronization
			DataSource dataSource = dataSources.get(jcd.getJcdAlias());
			if (dataSource == null) {
				dataSource = getDataSource(jcd.getJcdAlias());
				synchronized (dataSources) {
					dataSources.put(jcd.getJcdAlias(), dataSource);
				}
			}
			return dataSource.getConnection();
		} catch (Exception ex) {
			throw new LookupException("Could not obtain connection from data source: " + jcd.getJcdAlias(), ex);
		}
	}
