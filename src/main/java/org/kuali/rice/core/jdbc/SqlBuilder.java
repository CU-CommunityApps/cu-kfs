...
*/
  ...
  public static List<String> getCleanedSearchableValues(String valueEntered, String propertyDataType) {
   ...
   public static List<String> getSearchableValues(String valueEntered) {
		...
		int count = 0;
		getSearchableValueRecursive(valueEntered, lRet, count);

		return lRet;
	}

	protected static void getSearchableValueRecursive(String valueEntered, List lRet, int count) {
		...
		if (StringUtils.contains(valueEntered, KNSConstants.BETWEEN_OPERATOR)) {
			...
			for(String value : l){
				getSearchableValueRecursive(value,lRet, count);
			}
			return;
		}
