...
public class DisbursementVoucherAccountingLineAccessibleValidation extends AccountingLineAccessibleValidation {
    ...
    public boolean validate(AttributedDocumentEvent event) {
        ...
        boolean isAccessible = super.validate(event);
        ...
        if (!isAccessible && workflowDocument.stateIsEnroute()) {
        	...
        	if (workflowDocument.isApprovalRequested() && this.hasRequiredEditMode(accountingDocumentForValidation, currentUser, candidateEditModes)) {
        		isAccessible = true;
        		GlobalVariables.getMessageMap().removeAllErrorMessagesForProperty(KFSPropertyConstants.CHART_OF_ACCOUNTS_CODE);
        		GlobalVariables.getMessageMap().removeAllErrorMessagesForProperty(KFSPropertyConstants.ACCOUNT_NUMBER);
        	}
        }
