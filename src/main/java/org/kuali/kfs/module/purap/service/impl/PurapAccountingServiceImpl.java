...
public class PurapAccountingServiceImpl implements PurapAccountingService {
    ...
    public List<PurApAccountingLine> generateAccountDistributionForProration(List<SourceAccountingLine> accounts,
            KualiDecimal totalAmount, Integer percentScale) {
        return null;
    }
    ...
    public List<PurApAccountingLine> generateAccountDistributionForProration(List<SourceAccountingLine> accounts,
        ...
        if (totalAmount.isZero()) {
            throwRuntimeException("generateAccountDistributionForProration()", "Purchasing/Accounts Payable account " +
                    "distribution for proration does not allow zero dollar total.");
        }
        ...
        for (SourceAccountingLine accountingLine : accounts) {
            ...
            LOG.debug("generateAccountDistributionForProration() " + accountingLine.getAccountNumber() + " " +
                    amt + "/" + totalAmountBigDecimal);
            ...
            if (lowestPossible.compareTo(pct) <= 0) {
                ...
                LOG.debug("generateAccountDistributionForProration() adding " +
                        newAccountingLine.getAccountLinePercent());
        ...
        if (percentTotal.compareTo(BigDecimal.ZERO) == 0) {
            // This means there are so many accounts or so strange a distribution that we can't round properly... not
            // sure of viable solution
            throwRuntimeException("generateAccountDistributionForProration()", "Can't round properly due to " +
                    "number of accounts");
        }

        // Now deal with rounding
        if ((ONE_HUNDRED.compareTo(percentTotal)) < 0) {
            // The total percent is greater than one hundred Here we find the account that occurs latest in our list
            // with a percent that is higher than the difference and we subtract off the difference
            ...
            if (!foundAccountToUse) {
                // We could not find any account in our list where the percent of that account was greater than that
                // of the difference... doing so on just any account could result in a negative percent value
                throwRuntimeException("generateAccountDistributionForProration()", "Can't round properly due to " +
                        "math calculation error");
            }

        } else if ((ONE_HUNDRED.compareTo(percentTotal)) > 0) {
            // The total percent is less than one hundred Here we find the last account in our list and add the
            // remaining required percent to its already calculated percent
            ...
            slushAccount.setAccountLinePercent(slushLinePercent.add(difference).movePointLeft(2).stripTrailingZeros()
                    .movePointRight(2));
        }
        return newAccounts;
    }
    ...
    public List<PurApAccountingLine> generateAccountDistributionForProrationWithZeroTotal(
        ...
        if ((BigDecimal.ZERO.compareTo(totalPercentValue.remainder(ONE_HUNDRED))) != 0) {
            throwRuntimeException("generateAccountDistributionForProrationWithZeroTotal()",
                    "Invalid Percent Total of '" + totalPercentValue +
        ...
        for (PurApAccountingLine accountingLine : accounts) {
            ...
            LOG.debug("generateAccountDistributionForProrationWithZeroTotal() " +
                    accountingLine.getChartOfAccountsCode() + "-" + accountingLine.getAccountNumber() + " " + amt +
                    "/" + percentToUse);
            ...
            if (i != accountListSize || accountListSize == 1) {
                ...
                percentUsed = percentUsed.add(acctPercent.divide(totalPercentValue, SCALE, BIG_DECIMAL_ROUNDING_MODE)
                        .multiply(ONE_HUNDRED));
            } else {
            ...
            newAccountingLine.setAccountLinePercent(percentToUse.setScale(acctPercent.scale(),
                    BIG_DECIMAL_ROUNDING_MODE));

            LOG.debug("generateAccountDistributionForProrationWithZeroTotal() adding " +
                    newAccountingLine.getAccountLinePercent());
    ...
    public List<SourceAccountingLine> generateSummary(List<PurApItem> items) {
        ...
        return generateAccountSummary(items, null, ITEM_TYPES_EXCLUDED_VALUE, ZERO_TOTALS_RETURNED_VALUE,
                ALTERNATE_AMOUNT_NOT_USED, USE_TAX_INCLUDED, false);
    }
    ...
    public List<SourceAccountingLine> generateSummaryTaxableAccounts(List<PurApItem> items) {
        ...
        return generateAccountSummary(items, null, ITEM_TYPES_EXCLUDED_VALUE, ZERO_TOTALS_RETURNED_VALUE,
                ALTERNATE_AMOUNT_NOT_USED, USE_TAX_INCLUDED, true);
    }
     ...
     */
    protected List<SummaryAccount> generateSummaryAccounts(List<PurApItem> items, Boolean useZeroTotals,
            Boolean useTaxIncluded) {
        LOG.debug("generateSummaryAccounts() started");

        List<SummaryAccount> returnList = new ArrayList<>();

        List<SourceAccountingLine> sourceLines = generateAccountSummary(items, null,
                ITEM_TYPES_EXCLUDED_VALUE, useZeroTotals, ALTERNATE_AMOUNT_NOT_USED, useTaxIncluded, false);
            ...
            for (PurApItem item : items) {
                ...
                for (PurApAccountingLine purApAccountingLine : itemAccounts) {
                    if (purApAccountingLine.accountStringsAreEqual(summaryAccount.getAccount())) {
                        ...
                        // If the summaryItem is null, it means the item is not eligible to be displayed in the
                        // Account Summary tab. If it's not null then we'll set the estimatedEncumbranceAmount and add
                        // the item to the summaryAccount list to be displayed in the Account Summary tab.
                        KualiDecimal amt = KualiDecimal.ZERO;
    ...
    @Override
    public List<SummaryAccount> generateSummaryAccountsWithNoZeroTotalsNoUseTax(
    ...
    public List<SourceAccountingLine> generateSummaryWithNoZeroTotals(List<PurApItem> items) {
        ...
        return generateAccountSummary(items, null, ITEM_TYPES_EXCLUDED_VALUE,
                ZERO_TOTALS_NOT_RETURNED_VALUE, ALTERNATE_AMOUNT_NOT_USED, USE_TAX_INCLUDED, false);
    }
    ...
    public List<SourceAccountingLine> generateSummaryWithNoZeroTotalsNoUseTax(List<PurApItem> items) {
        ...
        return generateAccountSummary(items, null, ITEM_TYPES_EXCLUDED_VALUE,
                ZERO_TOTALS_NOT_RETURNED_VALUE, ALTERNATE_AMOUNT_NOT_USED, USE_TAX_EXCLUDED, false);
    }
    ...
    public List<SourceAccountingLine> generateSummaryWithNoZeroTotalsUsingAlternateAmount(List<PurApItem> items) {
        ...
        return generateAccountSummary(items, null, ITEM_TYPES_EXCLUDED_VALUE,
                ZERO_TOTALS_NOT_RETURNED_VALUE, ALTERNATE_AMOUNT_USED, USE_TAX_INCLUDED, false);
    }
    ...
    public List<SourceAccountingLine> generateSummaryExcludeItemTypes(List<PurApItem> items,
        ...
        return generateAccountSummary(items, excludedItemTypeCodes, ITEM_TYPES_EXCLUDED_VALUE,
                ZERO_TOTALS_RETURNED_VALUE, ALTERNATE_AMOUNT_NOT_USED, USE_TAX_INCLUDED, false);
    }
    ...
    public List<SourceAccountingLine> generateSummaryIncludeItemTypesAndNoZeroTotals(List<PurApItem> items,
        ...
        return generateAccountSummary(items, includedItemTypeCodes, ITEM_TYPES_INCLUDED_VALUE,
                ZERO_TOTALS_NOT_RETURNED_VALUE, ALTERNATE_AMOUNT_NOT_USED, USE_TAX_INCLUDED, false);
    }
    ...
    /**
     * Generates an account summary, that is it creates a list of source accounts by rounding up the purap accounts
     * off of the purap items.
     *
     * @param items                    the items to determ
     * @param itemTypeCodes            the item types to determine whether to look at an item in combination with
     *                                 itemTypeCodesAreIncluded
     * @param itemTypeCodesAreIncluded value to tell whether the itemTypeCodes parameter lists inclusion or exclusion
     *                                 variables
     ...
     */
    protected List<SourceAccountingLine> generateAccountSummary(List<PurApItem> items, Set<String> itemTypeCodes,
            Boolean itemTypeCodesAreIncluded, Boolean useZeroTotals, Boolean useAlternateAmount,
            Boolean useTaxIncluded, Boolean taxableOnly) {
        LOG.debug("generateAccountSummary() started");

        List<PurApItem> itemsToProcess = getProcessablePurapItems(items, itemTypeCodes, itemTypeCodesAreIncluded,
                useZeroTotals);
        Map<PurApAccountingLine, KualiDecimal> accountMap = new HashMap<>();
        ...
        for (PurApItem currentItem : itemsToProcess) {
            if (PurApItemUtils.checkItemActive(currentItem)) {
                ...
                if (taxableOnly) {
                    ...
                    if (!purapService.isTaxableForSummary(document.isUseTaxIndicator(),
                            purapService.getDeliveryState(document), currentItem)) {
                        continue;
                    }
                }

                if (!useTaxIncluded) {
                    // if no use tax set the source accounting lines to a clone so we can update them to be based on
                    // the non tax amount
                ...
                for (PurApAccountingLine account : sourceAccountingLines) {
                    ...
                    if (taxableOnly) {
                        ...
                        if (!purapService.isAccountingLineTaxable(account, purapService.isDeliveryStateTaxable(
                                purapService.getDeliveryState(document)))) {
                            continue;
                        }
                    }
        ...
        List<SourceAccountingLine> sourceAccounts = new ArrayList<>();
        while (iterator.hasNext()) {
            PurApAccountingLine accountToConvert = iterator.next();
    ...
    /**
     * This method takes a list of {@link PurApItem} objects and parses through them to see if each one should be
     * processed according the the other variables passed in.<br>
     * <br>
     * Example 1:<br>
     ...
     *
     * @param items                    list of {@link PurApItem} objects that need to be parsed
     * @param itemTypeCodes            list of {@link org.kuali.kfs.module.purap.businessobject.ItemType} codes used
     *                                 in conjunction with itemTypeCodesAreIncluded parameter
     * @param itemTypeCodesAreIncluded value to tell whether the itemTypeCodes parameter lists inclusion or exclusion
     *                                 variables (see {@link #ITEM_TYPES_INCLUDED_VALUE})
     * @param useZeroTotals            value to tell whether to include zero dollar items (see
     *                                 {@link #ZERO_TOTALS_RETURNED_VALUE})
     * @return a list of {@link PurApItem} objects that should be used for processing by calling method
     */
    protected List<PurApItem> getProcessablePurapItems(List<PurApItem> items, Set itemTypeCodes,
            Boolean itemTypeCodesAreIncluded, Boolean useZeroTotals) {
        ...
        if (!ITEM_TYPES_INCLUDED_VALUE.equals(itemTypeCodesAreIncluded)
            ...
            throwRuntimeException(methodName, "Invalid parameter found while trying to find processable items for " +
                    "dealing with purchasing/accounts payable accounts");
        }
        for (PurApItem currentItem : items) {
            if (itemTypeCodes != null && !itemTypeCodes.isEmpty()) {
                ...
                // check to see if item type code was found and if the list is describing included or excluded item
                // types
                if (foundMatchInList && ITEM_TYPES_EXCLUDED_VALUE.equals(itemTypeCodesAreIncluded)) {
                    // this item type code is in the list; this item type code is excluded so we skip it
                    // skips current item
                    continue;
                } else if (!foundMatchInList && ITEM_TYPES_INCLUDED_VALUE.equals(itemTypeCodesAreIncluded)) {
                    // this item type code is not in the list; this item type code is not included so we skip it
                    // skips current item
                    continue;
                }
            } else {
                ...
                if (ITEM_TYPES_INCLUDED_VALUE.equals(itemTypeCodesAreIncluded)) {
                    ...
                    throwRuntimeException(methodName, "Invalid parameter and list of items found while trying to " +
                            "find processable items for dealing with purchasing/accounts payable accounts");
                }
            }
    ...
    public void updateAccountAmounts(PurchasingAccountsPayableDocument document) {
        ...
        if ((document instanceof PaymentRequestDocument || document instanceof VendorCreditMemoDocument)
                && PurapConstants.AccountDistributionMethodCodes.SEQUENTIAL_CODE.equalsIgnoreCase(
        ...
        if ((document instanceof PaymentRequestDocument || document instanceof VendorCreditMemoDocument)
                && PurapConstants.AccountDistributionMethodCodes.PROPORTIONAL_CODE.equalsIgnoreCase(
                        accountDistributionMethod)) {
            // update the accounts amounts for PREQ and distribution method = sequential
            if (document instanceof VendorCreditMemoDocument) {
                VendorCreditMemoDocument cmDocument = (VendorCreditMemoDocument) document;
            ...
            for (PurApItem item : document.getItems()) {
                boolean rulePassed = kualiRuleService.applyRules(new PurchasingAccountsPayableItemPreCalculateEvent(
                        document, item));
        ...
        if (PurapConstants.AccountDistributionMethodCodes.SEQUENTIAL_CODE.equalsIgnoreCase(accountDistributionMethod)
                && !document.getItems().isEmpty()) {
            kualiRuleService.applyRules(new PurchasingAccountsPayableItemPreCalculateEvent(document,
                    document.getItems().get(0)));
            return;
        }
            ...
            for (T account : sourceAccountingLines) {
                if (ObjectUtils.isNotNull(account.getAccountLinePercent())
                    ...
                    if (ObjectUtils.isNotNull(account.getAmount())
                        ...
                        KualiDecimal calculatedPercent = new KualiDecimal(amt.multiply(new KualiDecimal(100))
                                .divide(totalAmount).toString());
                    ...
                    if (ObjectUtils.isNotNull(account.getAccountLinePercent())) {
                        ...
                        if (noDiscount) {
                            if (ObjectUtils.isNull(account.getAmount()) || account.getAmount().isZero()) {
                                account.setAmount(new KualiDecimal(pct.multiply(new BigDecimal(
                                        totalAmount.toString())).setScale(KualiDecimal.SCALE,
                                        KualiDecimal.ROUND_BEHAVIOR)));
                            }
                        } else {
                            account.setAmount(new KualiDecimal(pct.multiply(new BigDecimal(
                                    discountAmount.toString())).setScale(KualiDecimal.SCALE,
                                    KualiDecimal.ROUND_BEHAVIOR)));
                        }
                    }
                }
    ...
    /**
     * calculates values for a list of accounting lines based on an amount. Preq item's extended cost is distributed
     * to the accounting lines.
     *
     * @param sourceAccountingLines
     * @param totalAmount
     */
    @Override
    public <T extends PurApAccountingLine> void updatePreqAccountAmountsWithTotal(List<T> sourceAccountingLines,
            KualiDecimal totalAmount) {
        if ((totalAmount != null) && KualiDecimal.ZERO.compareTo(totalAmount) != 0) {
            KualiDecimal accountTotal = KualiDecimal.ZERO;
            T lastAccount = null;
            ...
            for (T account : sourceAccountingLines) {
                ...
                if (account.getAmount().isGreaterThan(KualiDecimal.ZERO)) {
                    if (totalAmount.isZero()) {
                        account.setAmount(KualiDecimal.ZERO);
                    } else if (account.getAmount().isGreaterThan(totalAmount)) {
            ...
            if (totalAmount.isGreaterThan(KualiDecimal.ZERO)) {
                for (T account : sourceAccountingLines) {
                    if (account.getAmount().isZero()
                            || account.getAccountLinePercent().compareTo(BigDecimal.ZERO) == 1) {
                        KualiDecimal priorAmount = account.getAmount();
                        account.setAmount(account.getAmount().add(new KualiDecimal(account.getAccountLinePercent())
                                .multiply(totalAmount).divide(new KualiDecimal(100))));
    ...
    public List<PurApAccountingLine> generatePercentSummary(PurchasingAccountsPayableDocument purapDoc) {
        List<PurApAccountingLine> accounts = new ArrayList<>();
                ...
                for (PurApAccountingLine account : currentItem.getSourceAccountingLines()) {
                    ...
                    for (PurApAccountingLine alreadyAddedAccount : accounts) {
                        if (alreadyAddedAccount.accountStringsAreEqual(account)) {
                            ...
                            alreadyAddedAccount.setAccountLinePercent(alreadyAddedAccountLinePercent.add(
                                    accountLinePercent));
        ...
        for (Object item : pr.getItems()) {
            ...
            if (paymentRequestItem.getTotalAmount() != null && paymentRequestItem.getTotalAmount().isNonZero()) {
                ...
                for (PurApAccountingLine purApAccountingLine : paymentRequestItem.getSourceAccountingLines()) {
                    ...
                    BigDecimal tmpPercent = accountAmount.bigDecimalValue().divide(extendedPrice.bigDecimalValue(),
                            PurapConstants.CREDITMEMO_PRORATION_SCALE, KualiDecimal.ROUND_BEHAVIOR);
                    ...
                    // test that the above amount is correct, if so just check that the total of all these matches
                    // the item total
                    ...
                    if (calcAmount.compareTo(accountAmount) != 0) {
                        ...
                        String param2 = calcAmount.bigDecimalValue()
                                .subtract(accountAmount.bigDecimalValue()).toString();
                        GlobalVariables.getMessageMap().putError(paymentRequestItem.getItemIdentifierString(),
    ...
    public List<SourceAccountingLine> generateSourceAccountsForVendorRemit(PurchasingAccountsPayableDocument document) {
        ...
        return generateSummaryWithNoZeroTotalsNoUseTax(document.getItems());
    }
    ...
    public List<UseTaxContainer> generateUseTaxAccount(PurchasingAccountsPayableDocument document) {
        List<UseTaxContainer> useTaxAccounts = new ArrayList<>();

        HashMap<PurApItemUseTax, UseTaxContainer> useTaxItemMap = new HashMap<>();
        ...
        for (UseTaxContainer useTaxContainer : useTaxAccounts) {
            ...
            List<SourceAccountingLine> origSourceAccounts = this.generateSummaryWithNoZeroTotals(
                    useTaxContainer.getItems());
            KualiDecimal totalAmount = calculateSumTotal(origSourceAccounts);
            List<PurApAccountingLine> accountingLines = generateAccountDistributionForProration(origSourceAccounts,
                    totalAmount, PurapConstants.PRORATION_SCALE, accountingLineClass);

            List<SourceAccountingLine> newSourceLines = new ArrayList<>();
            // convert back to source
    ...
    public boolean isTaxAccount(PurchasingAccountsPayableDocument document, SourceAccountingLine account) {
        ...
        if (document instanceof PaymentRequestDocument) {
            ...
            if (StringUtils.isNotEmpty(incomeClassCode)) {
                String federalChartCode = parameterService.getParameterValueAsString(PaymentRequestDocument.class,
                        NRATaxParameters.FEDERAL_TAX_PARM_PREFIX + NRATaxParameters.TAX_PARM_CHART_SUFFIX);
                String federalAccountNumber = parameterService.getParameterValueAsString(PaymentRequestDocument.class,
                        NRATaxParameters.FEDERAL_TAX_PARM_PREFIX + NRATaxParameters.TAX_PARM_ACCOUNT_SUFFIX);
                String federalObjectCode = parameterService.getSubParameterValueAsString(PaymentRequestDocument.class,
                        NRATaxParameters.FEDERAL_TAX_PARM_PREFIX +
                                NRATaxParameters.TAX_PARM_OBJECT_BY_INCOME_CLASS_SUFFIX, incomeClassCode);

                String stateChartCode = parameterService.getParameterValueAsString(PaymentRequestDocument.class,
                        NRATaxParameters.STATE_TAX_PARM_PREFIX + NRATaxParameters.TAX_PARM_CHART_SUFFIX);
                String stateAccountNumber = parameterService.getParameterValueAsString(PaymentRequestDocument.class,
                        NRATaxParameters.STATE_TAX_PARM_PREFIX + NRATaxParameters.TAX_PARM_ACCOUNT_SUFFIX);
                String stateObjectCode = parameterService.getSubParameterValueAsString(PaymentRequestDocument.class,
