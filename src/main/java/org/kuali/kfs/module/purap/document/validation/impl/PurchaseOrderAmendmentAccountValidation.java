...
public class PurchaseOrderAmendmentAccountValidation extends GenericValidation {
    ...
    public boolean validate(AttributedDocumentEvent event) {
        ...
        PurchaseOrderDocument po = purchaseOrderService
                .getCurrentPurchaseOrder(poaDocument.getPurapDocumentIdentifier());
                    ...
                    for (PurApAccountingLine accountingLine : accountingLines) {
                        if (accountingLine.getAccount().isExpired()) {
                            ...
                            GlobalVariables.getMessageMap().putError(PurapConstants.ITEM_TAB_ERROR_PROPERTY,
                                    PurapKeyConstants.ERROR_ITEM_ACCOUNT_EXPIRED,
    ...
    private boolean isItemChanged(PurApItem poaItem, List<PurApItem> poItems) {
        ...
        for (PurApItem poItem : poItems) {
            if (poItem.getItemTypeCode().equals(PurapConstants.ItemTypeCodes.ITEM_TYPE_ITEM_CODE)
                    && poaItemId == poItem.getItemLineNumber()) {
                // required fields so they cannot be null
                if (poaItem.getItemQuantity() == null
                        || poaItem.getItemQuantity().intValue() != poItem.getItemQuantity().intValue()) {
                    changed = true;
                }
                if (!poaItem.getItemUnitOfMeasureCode().equals(poItem.getItemUnitOfMeasureCode())) {
                    changed = true;
                }
                if (poaItem.getItemUnitPrice() == null
                        || poaItem.getItemUnitPrice().floatValue() != poItem.getItemUnitPrice().floatValue()) {
                    changed = true;
                }
