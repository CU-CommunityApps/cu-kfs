    ...
    public ActionForward refresh(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (StringUtils.equals(refreshCaller, "kimPersonLookupable")) {
            ...
            if (StringUtils.isNotEmpty(document.getDeliveryToName()) && document.getDeliveryToName().length() > deliveryToNameMaxLength.intValue()) {
                ...
                messageMap.addToErrorPath(PurapConstants.DELIVERY_TAB_ERRORS);
                messageMap.putWarning(PurapPropertyConstants.DELIVERY_TO_NAME, PurapKeyConstants.WARNING_DELIVERY_TO_NAME_TRUNCATED);
                messageMap.removeFromErrorPath(PurapConstants.DELIVERY_TAB_ERRORS);
            }
            ...
            if (StringUtils.isNotEmpty(document.getRequestorPersonName()) && document.getRequestorPersonName().length() > requestorNameMaxLength.intValue()) {
                ...
                messageMap.putWarning(PurapPropertyConstants.REQUESTOR_PERSON_NAME, PurapKeyConstants.WARNING_REQUESTOR_NAME_TRUNCATED);
                messageMap.removeFromErrorPath(PurapConstants.ADDITIONAL_TAB_ERRORS);
            }
        }
        ...
        if (StringUtils.equals(refreshCaller, VendorConstants.VENDOR_LOOKUPABLE_IMPL) && document.getVendorDetailAssignedIdentifier() != null && document.getVendorHeaderGeneratedIdentifier() != null) {
			...
			document.setPurchaseOrderTransmissionMethodCode(defaultAddress.getPurchaseOrderTransmissionMethodCode());

        }
        
        // KFSPTS-1719 : noqty is allowed for e-invoice.  the rule is removed.
