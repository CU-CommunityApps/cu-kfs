...
public class PaymentRequestServiceImpl implements PaymentRequestService {
	...
	public Collection<PaymentRequestDocument> getPaymentRequestsToExtractByCM(String campusCode,
		...
		List<PaymentRequestDocument> paymentRequestIterator = paymentRequestDao.getPaymentRequestsToExtract(campusCode,
				null, null, cmd.getVendorHeaderGeneratedIdentifier(), cmd.getVendorDetailAssignedIdentifier(),
				currentSqlDateMidnight);

		return filterPaymentRequestByAppDocStatus(paymentRequestIterator, PaymentRequestStatuses.APPDOC_AUTO_APPROVED,
	...
	public Collection<PaymentRequestDocument> getPaymentRequestsToExtractByVendor(String campusCode,
		...
		return filterPaymentRequestByAppDocStatus(paymentRequestDocuments, PaymentRequestStatuses.APPDOC_AUTO_APPROVED,
				PaymentRequestStatuses.APPDOC_DEPARTMENT_APPROVED);
	}
	...
	public Collection<PaymentRequestDocument> getPaymentRequestsToExtract(Date onOrBeforePaymentRequestPayDate) {
		...
		Collection<PaymentRequestDocument> paymentRequestIterator = paymentRequestDao.getPaymentRequestsToExtract(false,
				null, onOrBeforePaymentRequestPayDate);
	...
	public Collection<PaymentRequestDocument> getPaymentRequestsToExtractSpecialPayments(String chartCode,
		...
		Collection<PaymentRequestDocument> paymentRequestIterator = paymentRequestDao.getPaymentRequestsToExtract(true,
				chartCode, onOrBeforePaymentRequestPayDate);
		return filterPaymentRequestByAppDocStatus(paymentRequestIterator,
				PaymentRequestStatuses.STATUSES_ALLOWED_FOR_EXTRACTION);
	}
	...
	/**
	 * NOTE: in the event of auto-approval failure, this method may throw a
	 * RuntimeException, indicating to Spring transactional management that the
	...
	public boolean autoApprovePaymentRequest(String docNumber, KualiDecimal defaultMinimumLimit) {
		...
		try {
			...
			if (paymentRequestDocument.isHoldIndicator() || paymentRequestDocument.isPaymentRequestedCancelIndicator()
				...
				// PaymentRequestDaoOjb.getEligibleDocumentNumbersForAutoApproval() uses to
				// query the database.
				// Rechecking these conditions to ensure that the document is eligible for
				// auto-approval, because
				// we're not running things within the same transaction anymore and changes
				// could have occurred since
				// we called that method that make this document not auto-approvable

				// note that this block does not catch all race conditions however, this error
	...
	public boolean autoApprovePaymentRequest(PaymentRequestDocument doc, KualiDecimal defaultMinimumLimit) {
		if (isEligibleForAutoApproval(doc, defaultMinimumLimit)) {
			try {
				...
				// DocumentService.saveDocument are those that were dynamically created by
				// PojoFormBase (i.e., the
				// Document instance wasn't created from OJB). We need to make a deep copy and
				// materialize
				// collections to fulfill that assumption so that collection elements will
				// delete properly

				// TODO: maybe rewriting PurapService.calculateItemTax could be rewritten so
				// that the a deep copy
				...
				// set the auto approved indicator to true so that doRouteStatus method can use
				// to change the app doc
				// status.
				doc.setAutoApprovedIndicator(true);
				LOG.info("About to blanketApproveDocument, doc.getDocumentNumber()=" + doc.getDocumentNumber());
				// su approve rather than blanket approve, so no ACK notifications would be
				// generated
				...
				throw new RuntimeException(
						"Exception encountered when approving document number " + doc.getDocumentNumber() + ".", we);
			}
		}
		return true;
	}
	...
	/**
	 * Determines whether or not a payment request document can be automatically
	 * approved. FYI - If fiscal reviewers are allowed to save account changes
	 * without the full account validation running then this method must call full
	 * account validation to make sure auto approver is not blanket approving an
	 * invalid document according the the accounts on the items
	 *
	 * @param document            The payment request document to be determined
	 *                            whether it can be automatically approved.
	 * @param defaultMinimumLimit The amount to be used as the minimum amount if no
	 *                            limit was found or the default is less than the
	 *                            limit.
	 * @return boolean true if the payment request document is eligible for auto
	 *         approval.
	 */
	protected boolean isEligibleForAutoApproval(PaymentRequestDocument document, KualiDecimal defaultMinimumLimit) {
		...
		// This minimum will be set to the minimum limit derived from all accounting
		// lines on the document. If no
		// limit is determined, the default will be used.
		KualiDecimal minimumAmount = null;

		// Iterate all source accounting lines on the document, deriving a minimum limit
		// from each according to chart,
		...
		// If Receiving required is set, it's not needed to check the negative payment
		// request approval limit
		...
		// If no limit was found or the default is less than the limit, the default
		// limit is used.
		if (ObjectUtils.isNull(minimumAmount) || defaultMinimumLimit.compareTo(minimumAmount) < 0) {
			minimumAmount = defaultMinimumLimit;
		}

		// The document is eligible for auto-approval if the document total is below the
		// limit.
	...
	/**
	 * This method iterates a collection of negative payment request approval limits
	 * and returns the minimum of a given minimum amount and the least among the
	 * limits in the collection.
	 *
	 * @param limits        The collection of NegativePaymentRequestApprovalLimit to
	 *                      be used in determining the minimum limit amount.
	 * @param minimumAmount The amount to be compared with the collection of
	 *                      NegativePaymentRequestApprovalLimit to determine the
	 *                      minimum limit amount.
	 * @return The minimum of the given minimum amount and the least among the
	 *         limits in the collection.
	 */
	...
	/**
	 * Retrieves a list of payment request documents with the given vendor id and
	 * invoice number.
	 *
	...
	private HashMap<String, String> checkForDuplicatePaymentRequests(PaymentRequestDocument document) {
		...
		if (ObjectUtils.isNotNull(document.getInvoiceDate())) {
			if (purapService.isDateAYearBeforeToday(document.getInvoiceDate())) {
				msgs.put(KFSConstants.PaymentRequestDocumentConstants.DUPLICATE_INVOICE_QUESTION, configurationService
		...
		if (po != null) {
			msgs.putAll(checkForDuplicatesByVendorNumberAndInvoiceNumber(po.getVendorHeaderGeneratedIdentifier(),
					po.getVendorDetailAssignedIdentifier(), document.getInvoiceNumber(),
					PurapConstants.PREQDocumentsStrings.PAYEE_TOKEN, true));
	...
	public HashMap<String, String> checkForDuplicatePaymentRequests(Integer vendorHeaderGeneratedId,
		...
		messages.putAll(checkForDuplicatesByInvoiceNumberAndInvoiceDate(vendorHeaderGeneratedId, vendorDetailAssignedId,
				invoiceAmount, invoiceDate, specifiedSourceToken, questionFormat));
	 ...
	 */
	private Map<String, String> checkForDuplicatesByVendorNumberAndInvoiceNumber(final Integer vendorHeaderGeneratedId,
			final Integer vendorDetailAssignedId, final String invoiceNumber, final String vendorToken,
		...
		for (final PaymentRequestView possibleDuplicatePaymentRequest : possibleDuplicatePaymentRequests) {
			if (buildInvoiceNumberForComparison(possibleDuplicatePaymentRequest.getInvoiceNumber())
					.equals(invoiceNumberForComparison)) {
	...
	// strips special characters (anything not a-z, 0-9 or space) and converts to
	...
	private Map<String, String> checkForDuplicatesByInvoiceNumberAndInvoiceDate(Integer vendorHeaderGeneratedId,
		...
		return buildWarningMessagesForDuplicatesByInvoiceNumberAndAmount(duplicatePaymentRequests, specifiedSourceToken,
				DuplicatePaymentRequestMessages.DUPLICATE_INVOICE_DATE_AMOUNT, questionFormat);
	}
			...
			for (PaymentRequestDocument duplicatePaymentRequest : duplicatePaymentRequests) {
				if (StringUtils.equalsIgnoreCase(duplicatePaymentRequest.getApplicationDocumentStatus(),
					...
					if (foundVoided) {
						...
						messageText = configurationService
								.getPropertyValueAsString(duplicatePaymentRequestMessages.duplicateInvoiceCancelledKey);
						foundCanceled = true;
					}
					...
					if (foundCanceled) {
						...
						messageText = configurationService
								.getPropertyValueAsString(duplicatePaymentRequestMessages.duplicateInvoiceVoidedKey);
						foundVoided = true;
					}
				} else {
					messageText = configurationService
							.getPropertyValueAsString(duplicatePaymentRequestMessages.duplicateInvoiceKey);
					break;
				}
			}

			String formattedQuestion = questionFormat
					? configurationService.getPropertyValueAsString(FPKeyConstants.WARNING_DUPLICATE_INVOICE_QUESTION)
					: StringUtils.EMPTY;
	...
	public Map<String, String> getPaymentRequestsByStatusAndPurchaseOrderId(String applicationDocumentStatus,
		...
		// if there are no payment request document numbers exist then there is no need
		// to check for application
		...
		// helper method to filter the workflow documents that are created for Preq
		// documents. updates the map for
		// hasInProcess value to Y if records found for app doc status else sets the
		// value of checkInProcess = Y.
	...
	public java.sql.Date calculatePayDate(Date invoiceDate, PaymentTermType terms) {
		...
		// Retrieve pay date variation parameter (currently defined as 2). See parameter
	 ...
	 * processedDateCalendar.
	 *
	 * @param invoicedDateCalendar  One of the dates to be used in determining which
	 *                              date is later.
	 * @param processedDateCalendar The other date to be used in determining which
	 *                              date is later.
	 * @return The date which is the later of the two given dates in the input
	 *         parameters.
	 */
	...
	/**
	 * Calculates the paymentTermsDate given the dueTypeDescription,
	 * invoicedDateCalendar and the dueNumber.
	 *
	 ...
	 * @param dueNumber            Either the vendorDiscountDueNumber or the
	 *                             vendorDiscountDueNumber of the payment term.
	 */
	protected void paymentTermsDateCalculation(String dueTypeDescription, Calendar invoicedDateCalendar,
		...
		if (StringUtils.equals(dueTypeDescription, PurapConstants.PREQ_PAY_DATE_DATE)) {
			...
			// improper string
			throw new RuntimeException(
					"missing payment terms description or not properly entered on payment term " + "maintenance doc");
		}
	}
	 ...
	 *
	 * @param paymentRequestDocument The payment request document whose discount to
	 *                               be calculated.
	 */
	protected void calculateDiscount(PaymentRequestDocument paymentRequestDocument) {
		...
		if (pt != null && pt.getVendorPaymentTermsPercent() != null
			...
			if (discountItem == null) {
				// set discountItem and add to items this is probably not the best way of doing
				// it but should work for
			...
			if (salesTaxInd && !useTaxIndicator) {
				...
				discountItem.setItemTaxAmount(
						new KualiDecimal(discountTaxAmount.setScale(KualiDecimal.SCALE, KualiDecimal.ROUND_BEHAVIOR)));
			}
	...
	public void calculateTaxArea(PaymentRequestDocument preq) {
		...
		// don't need to calculate tax items if TaxClassificationCode is N
		// (Non_Reportable)
		if (StringUtils.equalsIgnoreCase(preq.getTaxClassificationCode(), "N")) {
			return;
		}

		// reserve the grand total excluding any tax amount, to be used as the base to
		// compute all tax items
		// if we don't reserve this, the pre-tax total could be changed as new tax items
		// are added
		BigDecimal taxableAmount = preq.getGrandPreTaxTotal().bigDecimalValue();

		// generate and add state tax gross up item and its accounting line, update
		// total amount,
		...
		// generate and add state tax item and its accounting line, update total amount,
		// if tax rate is non-zero
		...
		// generate and add federal tax gross up item and its accounting line, update
		// total amount,
		// if gross up indicator is true and tax rate is non-zero
		...
		// generate and add federal tax item and its accounting line, update total
		// amount, if tax rate is non-zero
		...
		// FIXME if user request to add zero tax lines and remove them after tax
		// approval,
		// then remove the conditions above when adding the tax lines, and
		// add a branch in PaymentRequestDocument.processNodeChange to call
		// PurapService.deleteUnenteredItems
	}

	/**
	 * Removes all existing Nonresident tax items from the specified payment
	 * request.
	 *
	...
	/**
	 * Generates a Nonresident tax item and adds to the specified payment request,
	 * according to the specified item type code.
	 *
	...
	protected void addTaxItem(PaymentRequestDocument preq, String itemTypeCode, BigDecimal taxableAmount) {
		...
		// set extended price amount as now it's calculated when accounting line is
		// generated
	 ...
	 *
	 * @param taxItem       The specified tax item the accounting line will be
	 *                      associated with.
	 * @param taxableAmount The amount to which tax is computed against.
	 * @return A fully populated PurApAccountingLine instance for the specified tax
	 *         item.
	 */
	protected PurApAccountingLine addTaxAccountingLine(PurApItem taxItem, BigDecimal taxableAmount) {
		...
		if (isGross) {
			// for gross up tax items, use preq's first item's first accounting line, which
			// shall exist at this point
			...
			taxAccount = parameterService.getParameterValueAsString(PaymentRequestDocument.class,
					NonresidentTaxParameters.FEDERAL_TAX_PARM_PREFIX
							+ NonresidentTaxParameters.TAX_PARM_ACCOUNT_SUFFIX);
			taxObjectCode = parameterService.getSubParameterValueAsString(PaymentRequestDocument.class,
					NonresidentTaxParameters.FEDERAL_TAX_PARM_PREFIX
							+ NonresidentTaxParameters.TAX_PARM_OBJECT_BY_INCOME_CLASS_SUFFIX,
			...
			taxObjectCode = parameterService.getSubParameterValueAsString(PaymentRequestDocument.class,
					NonresidentTaxParameters.STATE_TAX_PARM_PREFIX
							+ NonresidentTaxParameters.TAX_PARM_OBJECT_BY_INCOME_CLASS_SUFFIX,
					preq.getTaxClassificationCode());
			if (StringUtils.isBlank(taxChart) || StringUtils.isBlank(taxAccount)
					|| StringUtils.isBlank(taxObjectCode)) {
		...
		// calculate tax amount according to gross up indicator and federal/state tax
		// type
		/*
		 * The formula of tax and gross up amount are as follows: if (not gross up)
		 * gross not existing taxFederal/State = - amount * rateFederal/State otherwise
		 * gross up grossFederal/State = amount * rateFederal/State / (1 - rateFederal -
		 * rateState) tax = - gross
		 */
		...
		// tax is always negative, since it reduces the total amount; while gross up is
		// always the positive of tax
		if (!isGross) {
	 ...
	 *
	 * @param paymentRequestDocument The payment request document to be used to find
	 *                               the discount item.
	...
	protected void distributeAccounting(PaymentRequestDocument paymentRequestDocument) {
		...
		for (PaymentRequestItem item : (List<PaymentRequestItem>) paymentRequestDocument.getItems()) {
			...
			if (item.getSourceAccountingLines().isEmpty() && ObjectUtils.isNotNull(item.getExtendedPrice())
				...
				if (StringUtils.equals(ItemTypeCodes.ITEM_TYPE_PMT_TERMS_DISCOUNT_CODE,
					...
					// if summaryAccount is empty then do not call
					// generateAccountDistributionForProration as
		...
		// update again now that distribute is finished. (Note: we may not need this
		// anymore now that I added
		// updateItem line above leave the call below since we need to this when
		// sequential method is used on the
		// document.
		purapAccountingService.updateAccountAmounts(paymentRequestDocument);
	}
	 ...
	 *
	 * @param document The payment request document whose request cancel fields to
	 *                 be cleared.
	 */
	...
	public void populatePaymentRequest(PaymentRequestDocument paymentRequestDocument) {
		...
		// write a note for expired/closed accounts if any exist and add a message
		// stating there were expired/closed
		...
		// set indicator so a message is displayed for accounts that were replaced due
		// to expired/closed status
	...
	/**
	 * If the full document entry has been completed and the status of the related
	 * purchase order document is closed, return true, otherwise return false.
	 *
	 * @param apDoc The AccountsPayableDocument to be determined whether its
	 *              purchase order should be reversed.
	...
	public boolean shouldPurchaseOrderBeReversed(AccountsPayableDocument apDoc) {
		...
		return purapService.isFullDocumentEntryCompleted(apDoc)
				&& StringUtils.equalsIgnoreCase(PurchaseOrderStatuses.APPDOC_CLOSED, po.getApplicationDocumentStatus());
	}
	 ...
	 * @param preqDoc         The payment request document whose status to be
	 *                        updated.
	...
	protected String updateStatusByNode(String currentNodeName, PaymentRequestDocument preqDoc) {
		...
		if (StringUtils.isEmpty(currentNodeName)) {
			...
			cancelledStatus = PaymentRequestStatuses.getPaymentRequestAppDocDisapproveStatuses().get(currentNodeName);
		}
	...
	public boolean poItemEligibleForAp(AccountsPayableDocument apDoc, PurchaseOrderItem poi) {
		...
		if (poiType.isQuantityBasedGeneralLedgerIndicator()) {
			...
			// not quantity based
			// As long as it contains a number (whether it's 0, negative or positive
			// number), we'll have to return
			// true. This is so that the OutstandingEncumberedAmount and the Original Amount
			// from PO column would
	...
	public void changeVendor(PaymentRequestDocument preq, Integer headerId, Integer detailId) {
		...
		if (vd == null) {
			LOG.error("changeVendor() VendorDetail from database for header id " + headerId + " and detail id "
					+ detailId + "is null");
			throw new PurError("changeVendor: VendorDetail from database for header id " + headerId + " and detail id "
					+ detailId + "is null");
		}
		...
		VendorAddress va = vendorService.getVendorDefaultAddress(headerId, detailId, VendorConstants.AddressTypes.REMIT,
				deliveryCampus);
		if (va == null) {
			va = vendorService.getVendorDefaultAddress(headerId, detailId, VendorConstants.AddressTypes.PURCHASE_ORDER,
					deliveryCampus);
		}
		if (va == null) {
			LOG.error("changeVendor() VendorAddress from database for header id " + headerId + " and detail id "
					+ detailId + "is null");
			throw new PurError("changeVendor  VendorAddress from database for header id " + headerId + " and detail id "
					+ detailId + "is null");
		}
	...
	/**
	 * Records the specified error message into the Log file and throws a runtime
	 * exception.
	 *
	...
	public void generateGLEntriesCreateAccountsPayableDocument(AccountsPayableDocument apDocument) {
		...
		// JHK: this is not being injected because it would cause a circular reference
		// in the Spring definitions
	...
	public boolean hasActivePaymentRequestsForPurchaseOrder(Integer purchaseOrderIdentifier) {
		int activePaymentRequestCount = paymentRequestDao
				.getActivePaymentRequestCountForPurchaseOrder(purchaseOrderIdentifier);
		return activePaymentRequestCount > 0;
	}

	/**
	 * This method was added as part of the move to rice20 as a way to get at
	 * application doc status. Since this data has been moved back into KFS this
	 * function is no longer necessary. The code will be removed in the 6.0 release.
	 */
	@Deprecated
	 ...
	 * <p>
	 * This class first extract the payment request document numbers from the
	 * Payment Request Collections, then perform the
	 * filterPaymentRequestByAppDocStatus function. Base on the filtered payment
	...
	public boolean allowBackpost(PaymentRequestDocument paymentRequestDocument) {
		...
		// adding 1 to set the date to midnight the day after backpost is allowed so
		// that preqs allow backpost on the
		// last day
		...
		// if today is after the closing date but before/equal to the allowed backpost
		// date and the invoice date is
	...
	public boolean isPurchaseOrderValidForPaymentRequestDocumentCreation(PaymentRequestDocument paymentRequestDocument,
		...
		if (ObjectUtils.isNull(purchaseOrderDocument)) {
			...
			valid = false;
			// if the PO is pending and it is not a Retransmit, we cannot generate a Payment
			// Request for it
		}
	...
	public boolean encumberedItemExistsForInvoicing(PurchaseOrderDocument document) {
		...
		for (PurchaseOrderItem poi : (List<PurchaseOrderItem>) document.getItems()) {
			...
			if (poi.getItemType().isLineItemIndicator() && poi.getItemType().isQuantityBasedGeneralLedgerIndicator()) {
				KualiDecimal encumberedQuantity = poi.getItemOutstandingEncumberedQuantity() == null ? KualiDecimal.ZERO
	...
	private enum DuplicatePaymentRequestMessages {
		DUPLICATE_INVOICE_DATE_AMOUNT(PurapKeyConstants.MESSAGE_DUPLICATE_INVOICE_DATE_AMOUNT,
		...
		DUPLICATE_INVOICE_VENDOR_INVOICE_NUMBER(PurapKeyConstants.MESSAGE_DUPLICATE_INVOICE,
