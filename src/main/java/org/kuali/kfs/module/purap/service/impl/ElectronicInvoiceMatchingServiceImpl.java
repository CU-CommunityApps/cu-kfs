...
public class ElectronicInvoiceMatchingServiceImpl implements ElectronicInvoiceMatchingService {
    ...
    public void doMatchingProcess(ElectronicInvoiceOrderHolder orderHolder) {
        ...
        upperVariancePercentString = parameterService.getParameterValueAsString(ElectronicInvoiceStep.class,
                PurapParameterConstants.ElectronicInvoiceParameters.SALES_TAX_UPPER_VARIANCE_PERCENT);
        lowerVariancePercentString = parameterService.getParameterValueAsString(ElectronicInvoiceStep.class,
        ...
        try {
            ...
            ElectronicInvoiceRejectReason rejectReason = createRejectReason(
                    PurapConstants.ElectronicInvoice.INVALID_NUMBER_FORMAT, e.getMessage(), orderHolder.getFileName());
    ...
    protected void validateHeaderInformation(ElectronicInvoiceOrderHolder orderHolder) {
        ...
        if (orderHolder.isRejectDocumentHolder()) {
            VendorDetail vendorDetail = vendorService.getVendorByDunsNumber(orderHolder.getDunsNumber());
    ...
    protected void validateSummaryAmounts(ElectronicInvoiceOrderHolder orderHolder) {
        if (orderHolder.isRejectDocumentHolder()) {
            // If there are any rejects related to the summary, we're retaining it since it's not possible to get the
            // summary amount totals from the reject doc
            return;
        }
        ...
        boolean enableSalesTaxInd = parameterService.getParameterValueAsBoolean(
        ...
        // we handle bad PO's in the eInvoice later, so just skip this
        if (poDoc != null) {
    ...
    protected void validateItemMapping(ElectronicInvoiceOrderHolder orderHolder, String kualiItemTypeCode) {
        if (!orderHolder.isItemTypeAvailableInItemMapping(kualiItemTypeCode)) {
            ElectronicInvoiceRejectReason rejectReason = createRejectReason(
                    PurapConstants.ElectronicInvoice.ITEM_MAPPING_NOT_AVAILABLE, kualiItemTypeCode,
                    orderHolder.getFileName());
            orderHolder.addInvoiceHeaderRejectReason(rejectReason);
        }
    }
    ...
    protected void validateInvoiceItems(ElectronicInvoiceOrderHolder orderHolder) {
        ...
        if (itemHolders != null) {
            for (ElectronicInvoiceItemHolder itemHolder : itemHolders) {
                validateInvoiceItem(itemHolder, poLineNumbers);
            }
        }
    }
    ...
    protected void validateInvoiceItem(ElectronicInvoiceItemHolder itemHolder, Set poLineNumbers) {
        ...
        if (poItem == null) {
            ...
            ElectronicInvoiceRejectReason rejectReason = createRejectReason(
                    PurapConstants.ElectronicInvoice.NO_MATCHING_PO_ITEM, extraDescription, orderHolder.getFileName());
            orderHolder.addInvoiceOrderRejectReason(rejectReason,
        ...
        if (!poItem.isItemActiveIndicator()) {
            ...
            ElectronicInvoiceRejectReason rejectReason = createRejectReason(
                    PurapConstants.ElectronicInvoice.INACTIVE_LINE_ITEM, extraDescription, orderHolder.getFileName());
            orderHolder.addInvoiceOrderRejectReason(rejectReason,
                    PurapConstants.ElectronicInvoice.RejectDocumentFields.INVOICE_ITEM_LINE_NUMBER,
    ...
    protected void validateCatalogNumber(ElectronicInvoiceItemHolder itemHolder) {
        ...
        if (StringUtils.isNotBlank(invoiceCatalogNumberStripped)
            ...
            /*
             * If catalog number is empty in PO/&Invoice, check whether the catalog check is required for the
             * requisition source. If exists in param, create reject reason. If not exists, continue with UOM and unit
              * price match.
             */
            String reqSourceRequiringCatalogMatch = parameterService
    ...
    protected void validateQtyBasedItem(ElectronicInvoiceItemHolder itemHolder) {
        ...
        if (itemHolder.getInvoiceItemQuantity() == null) {
            ...
            String extraDescription = "Invoice Item Line Number:" + itemHolder.getInvoiceItemLineNumber();
            ElectronicInvoiceRejectReason rejectReason = createRejectReason(
                    PurapConstants.ElectronicInvoice.INVOICE_QTY_EMPTY, extraDescription, orderHolder.getFileName());
            orderHolder.addInvoiceOrderRejectReason(rejectReason,
                    PurapConstants.ElectronicInvoice.RejectDocumentFields.INVOICE_ITEM_QUANTITY,
            ...
            if (!itemHolder.getInvoiceOrderHolder().getPurchaseOrderDocument().isReceivingDocumentRequiredIndicator()) {
                if ((itemHolder.getInvoiceItemQuantity().compareTo(poItem.getItemOutstandingEncumberedQuantity().bigDecimalValue())) > 0) {
                    ...
                    String extraDescription = "Invoice Item Line Number:" + itemHolder.getInvoiceItemLineNumber();
                    ElectronicInvoiceRejectReason rejectReason = createRejectReason(
                            PurapConstants.ElectronicInvoice.PO_ITEM_QTY_LESSTHAN_INVOICE_ITEM_QTY, extraDescription,
                            orderHolder.getFileName());
                    orderHolder.addInvoiceOrderRejectReason(rejectReason,
                            PurapConstants.ElectronicInvoice.RejectDocumentFields.INVOICE_ITEM_QUANTITY,
    ...
    protected void validateSalesTax(ElectronicInvoiceItemHolder itemHolder) {
        ...
        boolean enableSalesTaxInd = parameterService
        ...
        KualiDecimal extendedPrice = new KualiDecimal(getExtendedPrice(itemHolder).setScale(KualiDecimal.SCALE,
    ...
    protected BigDecimal getExtendedPrice(ElectronicInvoiceItemHolder itemHolder) {
        ...
        } else if (ObjectUtils.isNotNull(itemHolder.getQuantity())) {
            // qty wont be null since it's defined as a reqd field in xsd
    ...
    protected Map<String, ElectronicInvoiceRejectReasonType> getElectronicInvoiceRejectReasonTypes() {
        Collection<ElectronicInvoiceRejectReasonType> collection = businessObjectService
                .findAll(ElectronicInvoiceRejectReasonType.class);
        Map rejectReasonTypesMap = new HashMap<String, ElectronicInvoiceRejectReasonType>();

        if (collection != null && collection.size() > 0) {
            ElectronicInvoiceRejectReasonType[] rejectReasonTypesArr =
                    new ElectronicInvoiceRejectReasonType[collection.size()];
            collection.toArray(rejectReasonTypesArr);
            for (ElectronicInvoiceRejectReasonType aRejectReasonTypesArr : rejectReasonTypesArr) {
                rejectReasonTypesMap.put(aRejectReasonTypesArr.getInvoiceRejectReasonTypeCode(),
                        aRejectReasonTypesArr);
            }
        }
