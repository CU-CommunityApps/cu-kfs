...
public class PurchasingUnitOfMeasureValidation extends GenericValidation {
    ...
    public boolean validate(AttributedDocumentEvent event) {
        ...
        if (purItem.getItemType().isQuantityBasedGeneralLedgerIndicator()) {
            String errorPrefix;
            if (ObjectUtils.isNotNull(itemForValidation.getItemLineNumber())) {
                errorPrefix = KFSPropertyConstants.DOCUMENT + "." + PurapPropertyConstants.ITEM + "[" + (itemForValidation.getItemLineNumber() - 1) + "]." + PurapPropertyConstants.ITEM_UNIT_OF_MEASURE_CODE;
            }
            else
            {
                errorPrefix = KFSPropertyConstants.DOCUMENT + "." + PurapPropertyConstants.ITEM + PurapPropertyConstants.ITEM_UNIT_OF_MEASURE_CODE; 
            }
