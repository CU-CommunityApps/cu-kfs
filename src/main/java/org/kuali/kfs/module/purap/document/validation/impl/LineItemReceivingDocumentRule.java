...
public class LineItemReceivingDocumentRule extends DocumentRuleBase implements ContinuePurapRule, AddReceivingItemRule{
    ...
    protected boolean processCustomRouteDocumentBusinessRules(Document document) {        
        ...
        valid &= validateQuantitiesReceivedNotExceedOrdered(lineItemReceivingDocument);
    ...
    public boolean processAddReceivingItemRules(ReceivingDocument document, LineItemReceivingItem item,String errorPathPrefix) {
        ...
        valid &= validatePoReceivingRequired(document);
    ...
    /*
     * KFSPTS-1769 : can not add new line if po is not receiving required
     */
    private boolean validatePoReceivingRequired(ReceivingDocument document) {
        GlobalVariables.getMessageMap().clearErrorPath();
        boolean valid = true;
        PurchaseOrderDocument pod = document.getPurchaseOrderDocument();
        if (pod != null && !pod.isReceivingDocumentRequiredIndicator()) {
            GlobalVariables.getMessageMap().putError(PurapConstants.ITEM_TAB_ERROR_PROPERTY, CUPurapKeyConstants.ERROR_ADD_NEW_RECEIVING_LINE);
            valid = false; 
        }
        return valid;
    }
