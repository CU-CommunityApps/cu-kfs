...
public class PurchaseOrderDocumentPresentationController extends
    ...
    public boolean canEdit(Document document) {
        ...
        if (!PurchaseOrderStatuses.APPDOC_IN_PROCESS.equals(poDocument.getApplicationDocumentStatus())
                && !PurchaseOrderStatuses.APPDOC_WAITING_FOR_DEPARTMENT.equals(
                        poDocument.getApplicationDocumentStatus())
                ...
                && !PurchaseOrderStatuses.APPDOC_AWAIT_PURCHASING_REVIEW.equals(
                        poDocument.getApplicationDocumentStatus())
                && !PurchaseOrderStatuses.APPDOC_AWAIT_NEW_UNORDERED_ITEM_REVIEW.equals(
                        poDocument.getApplicationDocumentStatus())
    ...
    public Set<String> getEditModes(Document document) {
        ...
        boolean salesTaxInd = getParameterService().getParameterValueAsBoolean(
        ...
        // INTERNAL PURCHASING ROUTE LEVEL - Approvers can edit full detail on Purchase Order except they cannot
        // change the CHART/ORG.
        if (poDocument.isDocumentStoppedInRouteNode(PurchaseOrderStatuses.NODE_CONTRACT_MANAGEMENT)) {
    ...
    /**
     * Determines whether to display the button to print the pdf for the first time transmit. Conditions: PO status is
     * Pending Print or the transmission method is changed to PRINT during the amendment.
     *
     ...
     * available for only a certain number of PO transmission types which are stored in a parameter (default to PRIN
     * and FAX)
     *
    ...
    protected boolean canPreviewPrintPo(PurchaseOrderDocument poDocument) {
        ...
        if (can) {
            List<String> methods = new ArrayList<>(getParameterService().getParameterValuesAsString(
                    PurchaseOrderDocument.class,
                    PurapParameterConstants.PURAP_PO_PRINT_PREVIEW_TRANSMISSION_METHOD_TYPES));
