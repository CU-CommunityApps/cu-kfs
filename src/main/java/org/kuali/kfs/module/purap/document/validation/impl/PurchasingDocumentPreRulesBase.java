...
public abstract class PurchasingDocumentPreRulesBase extends PurapDocumentPreRulesBase{
    ...
    /**
     * Routine determines whether the VendorAddress record for the chosen Vendor contains the data required for the
     * selected Method Of PO Transmission on the document. 
     */
    protected boolean confirmVendorDataRequiredForMethodOfPOTransmissionOnPurapDoc(Document document){
    	boolean dataIsValid = true;
         	 
    	//perform validation checks for the required data based on method of po transmission selected on document
    	if (document instanceof PurchasingDocumentBase) {
    		...
    		dataIsValid &= purchaseOrderTransmissionMethodDataRulesServiceImpl.validateDataForMethodOfPOTransmissionExistsOnVendorAddress(document); 
    	}

    	if ( (document instanceof PurchaseOrderDocument) && (!dataIsValid) ){
    		event.setActionForwardName(KFSConstants.MAPPING_BASIC);
            return false;
    	}
