...
public abstract class DetailSalarySettingAction extends SalarySettingBaseAction {
    ...
    public ActionForward addAppointmentFundingLine(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        boolean stateFundedRule = stateFundedAccountRule(newAppointmentFunding);
        if(!stateFundedRule){
            errorMap.putError(BCPropertyConstants.NEW_BCAF_LINE, CUBCKeyConstants.POSITION_NOT_ALLOWED_FOR_STATE_FUNDED_ACCOUNT, workingAppointmentFunding.getAccountNumber());
            return mapping.findForward(KFSConstants.MAPPING_BASIC);
        }
    ...
    /*
     * State funded accounts will have subfundgroup code of 'APSTAT'.
     * State funded accounts only support state approved positions.
     * State approved positions will have column  CU_STATE_CERT  in PS_POSITION_EXTRA table set 'Y' 
     */
    protected boolean stateFundedAccountRule(PendingBudgetConstructionAppointmentFunding newAppointmentFunding){
    	boolean retValue;
        String coaCode = newAppointmentFunding.getChartOfAccountsCode();
        String accountNumber = newAppointmentFunding.getAccountNumber();
        Account account = SpringContext.getBean(AccountService.class).getByPrimaryIdWithCaching(coaCode, accountNumber);
        String subFundGroupCode =account.getSubFundGroupCode();

        if(!subFundGroupCode.equalsIgnoreCase("APSTAT")){
        	// account is not state funded. 
            retValue = true;
        }
        else{
        
	        String positionNumber = newAppointmentFunding.getPositionNumber();
	        if(positionNumber== null || positionNumber.equalsIgnoreCase("")){
	        	retValue =  false;
	        }
	        else{
	        	
		        BudgetConstructionHumanResourcesPayrollInterfaceService budgetConstructionHumanResourcesPayrollInterfaceService = SpringContext.getBean(BudgetConstructionHumanResourcesPayrollInterfaceService.class);
		        String stateFunded = budgetConstructionHumanResourcesPayrollInterfaceService.getStateFundedCode(positionNumber);
		        
		        //
		        if(stateFunded == null) stateFunded = "N"; 
		        
		        if(stateFunded.equalsIgnoreCase("Y"))
		        	retValue =  true;
		        else
		        	retValue = false;
	        }
