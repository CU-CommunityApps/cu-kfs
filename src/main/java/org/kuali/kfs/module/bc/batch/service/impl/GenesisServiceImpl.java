...
public class GenesisServiceImpl implements GenesisService {
    ...
    public void bCUpdateStep(Integer BaseYear) {
        ...
        // if account not accessible add it to BC
        for (Map.Entry<String, String[]> missingAccount : missingAccounts.entrySet()) {
            String[] missingValues = missingAccount.getValue();
            String chartOfAccountsCode = missingValues[0];
            String accountNumber = missingValues[1];

            LOG.warn(String.format("    Adding (chart: %s, account: %s) in Budget Construction Accounting\n",
                    chartOfAccountsCode, accountNumber));

            boolean created = budgetDocumentService
                    .buildBudgetConstructionAccountReportsIfNeeded(accountNumber, chartOfAccountsCode);

            if (created) {
                LOG.warn(String.format(
                        "    (chart: %s, account: %s) successfully added in Budget Construction Account Reports\n",
                        chartOfAccountsCode, accountNumber));

                List<BudgetConstructionAccountOrganizationHierarchy> accountOrganizationHierarchy = budgetDocumentService
                        .retrieveOrBuildAccountOrganizationHierarchy(requestFiscalYear, chartOfAccountsCode,
                                accountNumber);

                if (accountOrganizationHierarchy != null) {
                    LOG.warn(String
                            .format("    (chart: %s, account: %s) successfully added in Budget Construction Account Organization Hierarchy\n",
                                    chartOfAccountsCode, accountNumber));
                } else {
                    LOG.warn(String
                            .format("    (chart: %s, account: %s) not successfully added in Budget Construction Account Organization Hierarchy\n",
                                    chartOfAccountsCode, accountNumber));
                }
            } else {
                LOG.warn(String.format(
                        "    (chart: %s, account: %s) not successfully added in Budget Construction Account Reports: it has a budget record level of 'N'\n",
                        chartOfAccountsCode, accountNumber));
            }
    ...
    /**
     * Sets the budgetDocumentService.
	...
	public BudgetDocumentService getBudgetDocumentService() {
