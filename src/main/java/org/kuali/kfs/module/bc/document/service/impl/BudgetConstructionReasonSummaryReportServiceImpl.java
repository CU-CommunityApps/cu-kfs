...
public class BudgetConstructionReasonSummaryReportServiceImpl implements BudgetConstructionReasonSummaryReportService {
    ...
    public void buildReportsBody(Integer fiscalYear, BudgetConstructionOrgReasonSummaryReport reasonSummaryReport, BudgetConstructionSalaryFunding salaryFundingEntry, BudgetConstructionSalarySocialSecurityNumber salarySSN) {
        ...
        if (ObjectUtils.isNotNull(appointmentFunding.getAppointmentTotalIntendedAmount()))
        	reasonSummaryReport.setAppointmentTotalIntendedAmount(appointmentFunding.getAppointmentTotalIntendedAmount().intValue());
        else
        	reasonSummaryReport.setAppointmentTotalIntendedAmount(0);
    ...
    protected void adjustPersonTotal(PersonTotalHolder totalsHolder) {
        ...
        // KFSPTS-1224 - Appears like some of these values could be null coming in from the DB so set them to 0 if they are null
        if (ObjectUtils.isNull(totalsHolder.salaryPayMonth)) totalsHolder.salaryPayMonth=0;
        if (ObjectUtils.isNull(totalsHolder.salaryNormalMonths)) totalsHolder.salaryNormalMonths=0;
        if (ObjectUtils.isNull(totalsHolder.salaryPercent)) totalsHolder.salaryPercent=BigDecimal.ZERO;
        if (ObjectUtils.isNull(totalsHolder.csfPayMonths)) totalsHolder.csfPayMonths=0;
        if (ObjectUtils.isNull(totalsHolder.csfPercent)) totalsHolder.csfPercent=BigDecimal.ZERO;
        if (ObjectUtils.isNull(totalsHolder.csfNormalMonths)) totalsHolder.csfNormalMonths=0;
        if (ObjectUtils.isNull(totalsHolder.csfAmount)) totalsHolder.csfAmount=0;
        ...
        if (totalsHolder.salaryPayMonth == 0 || totalsHolder.csfPayMonths == 0 || BigDecimal.ZERO.compareTo(totalsHolder.csfPercent) == 0 || totalsHolder.csfNormalMonths == 0) {
        	...
        	// KFSPTS-1224 - totalsHolder.salaryPayMonth can be 0 (due to being NULL in the DB and being converted to 0 above)
        	BigDecimal salaryMonthPercent = BigDecimal.ZERO;
        	if (!totalsHolder.salaryPayMonth.equals(0)) salaryMonthPercent = new BigDecimal(totalsHolder.salaryNormalMonths * 1.0 / totalsHolder.salaryPayMonth);
        	
            BigDecimal salaryFteQuantity = totalsHolder.salaryPercent.multiply(salaryMonthPercent);
            
        	// KFSPTS-1224 - totalsHolder.csfPayMonths can be 0 (due to being NULL in the DB and being converted to 0 above)
            BigDecimal csfMonthpercent = BigDecimal.ZERO;
            if (!totalsHolder.csfPayMonths.equals(0)) csfMonthpercent = new BigDecimal(totalsHolder.csfNormalMonths * 1.0 / totalsHolder.csfPayMonths);
            
            BigDecimal csfFteQuantity = totalsHolder.csfPercent.multiply(csfMonthpercent);

            // KFSPTS-1224 - csfFteQuantity can be 0 (due to being NULL in the DB and being converted to 0 above)
            BigDecimal restatementCsfPercent = BigDecimal.ZERO;;
            if ( !(csfFteQuantity.compareTo(BigDecimal.ZERO) == 0) )
            	restatementCsfPercent = salaryFteQuantity.divide(csfFteQuantity,2,RoundingMode.HALF_UP);
        ...
        if (totalsHolder.salaryPayMonth == 0) {
            ...
            if (!totalsHolder.salaryPayMonth.equals(0)) salaryFte = totalsHolder.salaryPercent.multiply(new BigDecimal(totalsHolder.salaryNormalMonths * 1.0 / (totalsHolder.salaryPayMonth * 100.0)));
        ...
        if (totalsHolder.salaryPayMonth != totalsHolder.csfPayMonths) {
            if (totalsHolder.csfPayMonths == 0) {
                ...
                if (!totalsHolder.csfPayMonths.equals(0)) amount = new BigDecimal(restatementCsfAmount * totalsHolder.salaryPayMonth * 1.0 / totalsHolder.csfPayMonths);
        ...
        for (BudgetConstructionSalaryFunding salaryFunding : salaryFundings) {
            ...
            if (!appointmentFunding.isAppointmentFundingDeleteIndicator()) {
                if (totalsHolder.curToInt <= -1) {
                	if (ObjectUtils.isNotNull(appointmentFunding.getAppointmentTotalIntendedAmount()))  //JIRA KFSPTS-1224
                		totalsHolder.curToInt = appointmentFunding.getAppointmentTotalIntendedAmount().intValue();
                	else
                		totalsHolder.curToInt = 0;
                }

                if (totalsHolder.curFteInt <= -1.00) {
                	if (ObjectUtils.isNotNull(appointmentFunding.getAppointmentTotalIntendedFteQuantity()))   //JIRA KFSPTS-1224
                		totalsHolder.curFteInt = appointmentFunding.getAppointmentTotalIntendedFteQuantity().doubleValue();
                	else
                		totalsHolder.curFteInt = 0;
                }
            }
        }
    }
