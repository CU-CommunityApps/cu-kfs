...
public class PositionSalarySettingAction extends DetailSalarySettingAction {
    ...
    public ActionForward loadExpansionScreen(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // KFSPTS-1293 - (See also IncumbentSalarySetting.java - loadExpansionScreen method) This issue is that after selecting a row from the list, any subsequent
        //   selections from that list display only the data from the first selection and not the one just selected.  This resolution is a workaround and as such
        //   is noted as obviously as possible here.  The specific issue is that data coming in from the list is not being propagated to this form class however
        //   the request object does contain the updated URL parameters and other form data.
        // BEGINNING OF WORKAROUND
        positionSalarySettingForm.setPositionNumber(request.getParameterValues("positionNumber")[0]);   // ********
        // END OF WORKAROUND
        MessageMap errorMap;
