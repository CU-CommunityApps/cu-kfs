...
public class CertificationReportAction extends EffortCertificationAction {
    ...
    public ActionForward reviewPercentagesDetail(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
      ...
      //  System.out.println("ReviewPercentagesDetail");
        ...
        for (EffortCertificationDetail detailLine : detailLines){
            ...
            String errorPathPrefix = KFSPropertyConstants.DOCUMENT + "." + EffortPropertyConstants.EFFORT_CERTIFICATION_DETAIL_LINES;
            boolean isValid = this.invokeRules(new CheckDetailLineAmountEvent("", errorPathPrefix, effortDocument, detailLine));
            if (isValid) {
                KualiDecimal totalPayrollAmount = effortDocument.getTotalOriginalPayrollAmount();
    ...
    public ActionForward reviewPercentagesSummary(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
       ...
       // System.out.println("ReviewPercentagesSummary");
        ...
        int lineToRecalculateIndex=0;
        for (EffortCertificationDetail summarizedDetailLine : summarizedDetailLines){
            EffortCertificationDocumentRuleUtil.applyDefaultValues(summarizedDetailLine);
            ...
            boolean isValid = this.invokeRules(new CheckDetailLineAmountEvent("", errorPathPrefix, effortDocument, summarizedDetailLine));
            if (isValid) {
                summarizedDetailLine.recalculatePayrollAmount(totalPayrollAmount);

                String groupId = summarizedDetailLine.getGroupId();
                ...
                for (EffortCertificationDetail line : detailLinesToRecalculate) {
                    ObjectUtil.buildObject(line, summarizedDetailLine, EffortConstants.DETAIL_LINES_GROUPING_FILEDS);
                }
                ...
                DetailLineGroup detailLineGroup = detailLineGroupMap.get(DetailLineGroup.getKeysAsString(summarizedDetailLine));
                this.updateDetailLineGroup(detailLineGroup, summarizedDetailLine, totalPayrollAmount);
    ...
    public ActionForward reviewTotalsSummary(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        
       // System.out.println("ReviewTotalsSummary");
        ...
        int lineToRecalculateIndex=0;
        for (EffortCertificationDetail summarizedDetailLine : summarizedDetailLines){
            KualiDecimal amt = summarizedDetailLine.getEffortCertificationPayrollAmount();
            Double percentage = ((amt.doubleValue()/totalPayrollAmount.doubleValue()) * 100.00) +0.50;
            summarizedDetailLine.setEffortCertificationUpdatedOverallPercent(percentage.intValue());
    ...
    public ActionForward reviewTotalsDetail(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
     ...
     //   System.out.println("ReviewTotalsDetail");
        ...
        for (EffortCertificationDetail summarizedDetailLine : detailLines){
            KualiDecimal amt = summarizedDetailLine.getEffortCertificationPayrollAmount();
            Double percentage = ((amt.doubleValue()/totalPayrollAmount.doubleValue()) * 100.00) +0.50;
            summarizedDetailLine.setEffortCertificationUpdatedOverallPercent(percentage.intValue());
