...
public class PaymentApplicationDocument extends GeneralLedgerPostingDocumentBase implements
    ...
    protected List<GeneralLedgerPendingEntry> createPendingEntries(
        ...
        for (final InvoicePaidApplied ipa : appliedPayments) {
            ...
            // CU Customization Workaround for accountsReceivableObjectCode PersistenceBrokerException
            try {
                ...
                LOG.error("createPendingEntries: error getting ObjectCode from CustomerInvoiceDetail", ex);

                String accountsReceivableObjectCodeString = ipa.getInvoiceDetail().getAccountsReceivableObjectCode();
                ObjectCode arObjectCodeRef = forceGetObjectCode(processingChartCode, accountsReceivableObjectCodeString, invoiceObjectCode);

                offsetCreditEntry.setFinancialObjectCode(accountsReceivableObjectCodeString);
                offsetCreditEntry.setFinancialObjectTypeCode(arObjectCodeRef.getFinancialObjectTypeCode());
            }
    ...
    // CU Customization Get the Object Code Reference Manually
    private ObjectCode forceGetObjectCode(String chartCode, String arObjectCode, ObjectCode invoiceObjectCodeRef) {
        if (StringUtils.equals(arObjectCode, invoiceObjectCodeRef.getFinancialObjectCode()) &&
                StringUtils.equals(chartCode, invoiceObjectCodeRef.getChartOfAccountsCode()) &&
                getPostingYear().equals(invoiceObjectCodeRef.getUniversityFiscalYear())) {
            return invoiceObjectCodeRef;
        } else {
            final Map<String, String> keys = new HashMap<>();
            keys.put("chartOfAccountsCode", chartCode);
            keys.put("universityFiscalYear", Integer.toString(getPostingYear()));
            keys.put("financialObjectCode", arObjectCode);
            Object objectCode = getBusinessObjectService().findByPrimaryKey(ObjectCode.class, keys);
            return (ObjectCode) objectCode;
        }
    }
