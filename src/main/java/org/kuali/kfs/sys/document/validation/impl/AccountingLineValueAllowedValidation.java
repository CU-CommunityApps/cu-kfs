...
public class AccountingLineValueAllowedValidation extends GenericValidation {
    ...
    // JHK: Yes, I know this has a problem...when the thread is re-used, the cache could already be populated
    // But, how often are these values changed in production.
    static ThreadLocal<Map<Class,Map<String,ReusableParameterEvaluator>>> parameterCache = new ThreadLocal<Map<Class,Map<String,ReusableParameterEvaluator>>>();
    
    protected ReusableParameterEvaluator getParameterEvaluator( Class<? extends Object> componentClass, String parameterName ) {
    	// init the thread's cache if needed
    	Map<Class,Map<String,ReusableParameterEvaluator>> cache = parameterCache.get();
    	if ( cache == null ) {
    		cache = new HashMap<Class, Map<String,ReusableParameterEvaluator>>();
    	...
    	// main param section
    	ReusableParameterEvaluator parameterEvaluator = classParameters.get(parameterName);
    	// if the key truely does not exist, then see if one does
    	// if the key exists, then we cached a null and can let it remain
    	if ( parameterEvaluator == null && !classParameters.containsKey(parameterName) ) {
        	if ( getParameterService().parameterExists(componentClass, parameterName) ) {
        		parameterEvaluator = new ReusableParameterEvaluator( (ParameterEvaluatorImpl) getParameterService().getParameterEvaluator(componentClass, parameterName, "") );
        	}
        	classParameters.put(parameterName, parameterEvaluator);
    	}
    ...
    private boolean isAccountingLineValueAllowed(Class documentClass, AccountingLine accountingLine, String parameterName, String propertyName, String userEnteredPropertyName) {
        ...
        try {
            ...
            ReusableParameterEvaluator globalParameterEvaluator = getParameterEvaluator(KfsParameterConstants.FINANCIAL_PROCESSING_DOCUMENT.class, parameterName);
            ReusableParameterEvaluator documentParameterEvaluator = getParameterEvaluator(documentClass, parameterName);
            
            if ( globalParameterEvaluator != null ) {
            	isAllowed = globalParameterEvaluator.evaluateAndAddError(propertyValue, SourceAccountingLine.class, propertyName, userEnteredPropertyName);
            }
            if ( documentParameterEvaluator != null ) {
            	isAllowed = documentParameterEvaluator.evaluateAndAddError(propertyValue, SourceAccountingLine.class, propertyName, userEnteredPropertyName);
            }
        }
