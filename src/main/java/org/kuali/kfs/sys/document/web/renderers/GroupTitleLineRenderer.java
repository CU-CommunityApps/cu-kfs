...
public class GroupTitleLineRenderer implements Renderer, CellCountCurious {
    ...
    public void render(PageContext pageContext, Tag parentTag) throws JspException {
        try {
            ...
            if (canEdit && (getAccountingDocument() instanceof RequisitionDocument) && CollectionUtils.isNotEmpty(accounts.getKeyValues()) && accounts.getKeyValues().size() > 1) {
    ...
    private String buildFavoriteAccounts(String accountPrefix) {
        ...
        if (getAccountingDocument() instanceof RequisitionDocument) {
        	if (itemIdx >= 0) {
        		accountLineId = ((RequisitionItem)((RequisitionDocument)this.getAccountingDocument()).getItem(itemIdx)).getFavoriteAccountLineIdentifier();
        	} else {
        		accountLineId = ((RequisitionDocument)this.getAccountingDocument()).getFavoriteAccountLineIdentifier();
        	}
        }
