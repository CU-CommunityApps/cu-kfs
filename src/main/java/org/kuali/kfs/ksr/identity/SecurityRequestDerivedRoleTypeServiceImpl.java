package org.kuali.kfs.ksr.identity;
...
import org.kuali.rice.kim.api.KimConstants.AttributeConstants;
 ...
 * CU Customizations:
 * 
 * Copied over the SecurityRequestDerivedRoleTypeServiceImpl class from a more up-to-date rSmart KSR repository,
 * and tweaked it to directly use a log4j logger instead of using some intermediate helper class.
 * 
 * Fixed an area of the code where it was possible for inactive provisioning groups to be used by mistake.
 * 
 * Updated distributed authorizer routing so that request role qualifications with all-blank-value attributes
 * will no longer be used to find potential recipients.
 * 
 * Updated distributed authorizer routing for non-qualified request roles, which now uses primary department code
 * routing instead of blank qualification routing.
 * 
 * Updated Additional and Central Authorizer routing to behave like Distributed Authorizer routing when
 * those authorizer roles are qualified.
 * 
 * Cleaned up one of the getRoleMembersFromDerivedRole() methods to remove unused arguments.
 * 
 * Also remediated this class as needed for Rice 2.x compatibility.
 * ====
 * 
 * Derived role type service for the security request authorizer route levels
 * <p>
 * Members of the derived role are determined by the security provisioning record that corresponds to the role being
 * requested on the security request document. This role type is used for the distributed, additional, and central
 * routing levels for the document.
 * </p>
 * 
...
public class SecurityRequestDerivedRoleTypeServiceImpl extends DerivedRoleTypeServiceBase {
    ...
    protected String getAuthorizerRoleId(final String roleName, final SecurityRequestDocument document, final SecurityRequestRole requestRole) {
        ...
        final SecurityProvisioningGroup provisioningGroup = getProvisioningGroupForRequest(document.getSecurityGroupId(),
                                                                                           requestRole);
    ...
    protected List<RoleMembership> getRoleMembersFromDistributedAuthorizerRole(final String authorizerRoleId, final SecurityRequestDocument document, final SecurityRequestRole requestRole) {
        ...
        for (final SecurityRequestRoleQualification roleQualification : requestRole.getRequestRoleQualifications()) {
            Map<String,String> roleQualifier = roleQualification.buildQualificationMap();
            ...
            if (hasNonBlankQual) {
            	...
            	if (addMember) {
            		if (LOG.isTraceEnabled()) {
            			LOG.trace("Adding member role id: " + authorizerRoleId + " with qualification: "
            					+ roleQualifier.toString());
            		}
    ...
    public List<RoleMembership> getRoleMembersFromDerivedRole(
            String roleName, SecurityRequestDocument document, SecurityRequestRole requestRole) {
        ...
        if (KsrConstants.SECURITY_REQUEST_DISTRIBUTED_AUTHORIZER_ROLE_NAME.equals(roleName)) {
            ...
            if (addMember) {
                ...
                List<RoleMembership> authorizerRoleMembers = getAuthorizerRoleMembers(authorizerRoleId,
    ...
    /**
     * Determines if the given role id and qualification set have already been added to the map of added members, if not
     * the role and qualification are added
     * 
     * @param addedMembers
     ...
     * @return boolean true if the role id and qualification have been added to the map and should be added as members,
     *         false if there is already a member
     */
    protected boolean addMemberIfNotPreviouselyAdded(Map<String, List<Map<String,String>>> addedMembers, String roleId,
                                                     Map<String,String> qualification) {
    ...
    /**
     * Retrieves the security provisioning group record configured for the given security group id and role id contained
     * on the given security request role record
     * 
     ...
     */
    protected SecurityProvisioningGroup getProvisioningGroupForRequest(Long securityGroupId,
                                                                       SecurityRequestRole requestRole) {
        QueryByCriteria criteria = QueryByCriteria.Builder.fromPredicates(
                ...
                PredicateFactory.equal(KsrConstants.PROVISIONING_ROLE_ID, requestRole.getRoleId()),
        ...
        SecurityProvisioningGroup provisioningGroup = getDataObjectService()
            .findUnique(SecurityProvisioningGroup.class, criteria);
    ...
    protected boolean isQualifiedAuthorizerRole(String authorizerRoleId) {
    	Role role = KimApiServiceLocator.getRoleService().getRole(authorizerRoleId);
