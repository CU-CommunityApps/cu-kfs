 ...
 * ====
 * CU Customization:
 * Remediated this class as needed for Rice 2.x compatibility, including the following:
 * 
 * Moved validation of duplicate dependent roles (for addLine case) into the ProvisioningGroup maintenance view beans.
 * Updated provisioned-role validation to clean up the logic for verifying that the role is not a derived one.
 * Updated authorization role validation to perform proper comparisons and blank-value checks.
 * ====
 * 
 * Validation for the SecurityProvisioningGroup maintenance document
 * 
...
public class SecurityProvisioningGroupRule extends MaintenanceDocumentRuleBase {
	...
	/**
	 * Check to see if a SecurityProvisioningGroupDependentRoles object entered exists, and if it does, if it is derived.
	 * 
	...
	/**
	 * Check to see the there are any duplicate role ID's in other Security Provisioning Groups
	 * 
	 * @param document
	 * 			  -The current maintenance document
	 * @param securityProvisioningGroup
	 *            -Security Provisioning Group to check
	...
	/**
	 * Overloaded version of isDuplicateSecurityProvisioningGroup Check to see the there are any duplicate role ID's in other Security Provisioning Groups
	 * 
	 * @param document
	 * 			  -The current maintenance document
	 * @param securityProvisioningGroup
	 *            -Security Provisioning Group to check
	 * @param index
	 *            -index of the Security Provisioning Group in the document.
	 * @return -true if no match is found.
	 */
		...
		for (int i = 0; i < securityProvisioning.getSecurityProvisioningGroups().size(); i++) {
			...
			if (securityProvisioning.getSecurityProvisioningGroups().get(i).getRoleId().equals(securityProvisioningGroup.getRoleId())) {
				...
				if (map.getErrorPath().size() == 0) {
					newErrorPath = true;
					GlobalVariables.getMessageMap().addToErrorPath(KsrConstants.KSR_DOCUMENT_MAINTANABLE);
					GlobalVariables.getMessageMap().addToErrorPath(errorPathPrefix);
				}
				if ((index == -1) || (index > i)) {
					GlobalVariables.getMessageMap().putError(KsrConstants.PROVISIONING_ROLE_ID, KsrConstants.ERROR_SECURITY_PROVISIONING_GROUP_ROLE_UNIQUE, new String[] { securityProvisioningGroup.getRole().getName() });
				}
				if (newErrorPath) {
					GlobalVariables.getMessageMap().removeFromErrorPath(KsrConstants.KSR_DOCUMENT_MAINTANABLE);
	...
	/**
	 * Find duplicate dependent roles
	 * 
	 * @param document
	 * 			  -The current maintenance document
	 * @param dependentRole
		...
		for (int i = 0; i < securityProvisioningGroup.getDependentRoles().size(); i++) {
			...
			if (securityProvisioningGroup.getDependentRoles().get(i).getRoleId().equals(dependentRole.getRoleId())) {
				...
				if ((indexRole == -1) || (indexRole > i)) {
					GlobalVariables.getMessageMap().putError(KsrConstants.PROVISIONING_ROLE_ID, KsrConstants.ERROR_SECURITY_PROVISIONING_GROUP_DEPENDENT_ROLE_UNIQUE, new String[] { dependentRole.getRole().getName() });
				}
				if (newErrorPath) {
					GlobalVariables.getMessageMap().removeFromErrorPath(KsrConstants.KSR_DOCUMENT_MAINTANABLE);
					GlobalVariables.getMessageMap().removeFromErrorPath(errorPathPrefix);
	...
	/**
	 * Validate a dependent role
	 * 
	 * @param dependentRole
	 *            -The Dependent Role to be validated.
	...
	/**
	 * Overloaded version of validateDependentRole Validate a dependent role
	 * 
	 * @param dependentRole
	 *            -The Dependent Role to be validated
	 * @param index
	 *            -The index of the SecurityProvisioningGroup the Dependent Role belongs to.
	 * @param indexRole
	 *            -The index of the Dependent Role
	 * @return -true if it passes validation
	 */
	private boolean validateDependentRole(SecurityProvisioningGroupDependentRoles dependentRole, int index, int indexRole) {
		...
		if (StringUtils.isBlank(dependentRole.getRoleId())) {
			...
			GlobalVariables.getMessageMap().putError(KsrConstants.PROVISIONING_ROLE_ID, KsrConstants.ERROR_SECURITY_PROVISIONING_GROUP_DEPENDENT_ROLE_BLANK);
	...
	/**
	 * validate new and current SecurityProvisioningGroups
	 * 
	 * @param document
	 * 			  -The current maintenance document
	 * @param securityProvisioningGroup
	 *            -The SecurityProvisioningGroup to validate
	 * @param index
	 *            -The index of the SecurityProvisioningGroup in the document's list of SecurityProvisioningGroups
	...
	/**
	 * Overloaded version of validateSecurityProvisioningGroup validate new and current SecurityProvisioningGroups
	 * 
	 * @param document
	 * 			  -The current maintenance document
	 * @param securityProvisioningGroup
	...
	private boolean validateSecurityProvisioningGroup(MaintenanceDocument document, SecurityProvisioningGroup securityProvisioningGroup, int index) {
		...
		Role role = getRoleService().getRole(securityProvisioningGroup.getRoleId());
		if (role != null) {
			...
			if (kimTypeService != null && kimTypeService instanceof org.kuali.rice.kns.kim.role.DerivedRoleTypeServiceBase) {
				success = false;
				MessageMap map = GlobalVariables.getMessageMap();
				String errorPathPrefix = (index != -1 ? "" : "add.")
						+ KsrConstants.SECURITY_PROVISIONING_GROUPS
				...
				GlobalVariables.getMessageMap().putError(KsrConstants.SECURITY_PROVISIONING_GROUP_ROLE_NAME,
				        KsrConstants.ERROR_SECURITY_PROVISIONING_GROUP_ROLE_DERIVED, new String[] { securityProvisioningGroup.getRole().getName() });
		...
		if (success) {
			...
			if (StringUtils.isNotBlank(securityProvisioningGroup.getDistributedAuthorizerRoleId())) {
				if (StringUtils.isNotBlank(securityProvisioningGroup.getAdditionalAuthorizerRoleId())) {
					if (securityProvisioningGroup.getDistributedAuthorizerRoleId().equals(securityProvisioningGroup.getAdditionalAuthorizerRoleId())) {
						...
						GlobalVariables.getMessageMap().putError(KsrConstants.SECURITY_PROVISIONING_GROUP_ADD_AUTH_ROLE_ID, KsrConstants.ERROR_SECURITY_PROVISIONING_GROUP_AUTH_UNIQUE, new String[] {
				...
				if (StringUtils.isNotBlank(securityProvisioningGroup.getCentralAuthorizerRoleId())) {
					if (securityProvisioningGroup.getDistributedAuthorizerRoleId().equals(securityProvisioningGroup.getCentralAuthorizerRoleId())) {
						...
						GlobalVariables.getMessageMap().putError(KsrConstants.SECURITY_PROVISIONING_GROUP_CENT_AUTH_ROLE_ID, KsrConstants.ERROR_SECURITY_PROVISIONING_GROUP_AUTH_UNIQUE, new String[] {
								KsrConstants.SECURITY_PROVISIONING_GROUP_CENT_AUTH_LBL,
								KsrConstants.SECURITY_PROVISIONING_GROUP_DIST_AUTH_LBL,
								KsrConstants.SECURITY_PROVISIONING_GROUP_ADD_AUTH_LBL });
						if (newErrorPath) {
			...
			else if (StringUtils.isNotBlank(securityProvisioningGroup.getAdditionalAuthorizerRoleId())) {
				if (StringUtils.isNotBlank(securityProvisioningGroup.getCentralAuthorizerRoleId())) {
		...
		for (int i = 0; i < securityProvisioningGroupList.size(); i++) {
			...
			if (securityProvisioningGroup.getTabId().equals(securityProvisioningGroupList.get(i).getTabId())) {
				if (securityProvisioningGroup.getRoleTabOrder() == securityProvisioningGroupList.get(i).getRoleTabOrder()) {
					...
					GlobalVariables.getMessageMap().putError(KsrConstants.PROVISIONING_ROLE_TAB_ORDER, KsrConstants.ERROR_SECURITY_PROVISIONING_GROUP_TAB_ORDER_UNIQUE);
	...
	/**
	 * Determine whether to show the index of a collection or not for an error
	 * 
	 * @param index
	 *            -the index to be used
	 * @return -blank String or [0], [1], ...[n]
	 */
	...
	/**
	 * Validate the SecurityProvisioningGroup.
	 * 
		...
		if (success) {
			...
			for (int i = 0; i < securityProvisioning.getSecurityProvisioningGroups().size(); i++) {
				...
				if (matches > 1) {
					...
					GlobalVariables.getMessageMap().putError(KsrConstants.SECURITY_PROVISIONING_GROUP_ROLE_NAME, KsrConstants.ERROR_SECURITY_PROVISIONING_GROUP_CIRCULAR_REFERENCE, new String[] { securityProvisioning.getSecurityProvisioningGroups().get(i).getRole().getName() });
	...
	/**
	 * Checks that a dependent role has a corresponding active SecurityProvisioningGroup.
	 * 
	 * @param document
	 * 			  -The current maintenance document
	 * @param dependentRole
	 *            -the Dependent Role to be checked
	 * @param index
	 *            -The index of the SecurityProvisioningGroup
	 * @param indexRole
	 *            -The index of the Dependent Role within the SecurityProvisioningGroup
	...
	private boolean hasSecurityProvisioningGroup(MaintenanceDocument document, SecurityProvisioningGroupDependentRoles dependentRole, int index, int indexRole) {
		...
		if (!success) {
			...
			GlobalVariables.getMessageMap().putError(KsrConstants.PROVISIONING_ROLE_ID, KsrConstants.ERROR_SECURITY_PROVISIONING_GROUP_DEPENDENT_ROLE_MATCH, new String[] { dependentRole.getRole().getName() });
	...
	/**
	 * Go through a SecurityProvisioningGroup and create a string of it's roleID and it's dependent roleID.
	 * 
	 * @param roleID
	 *            -the role ID to build a tree string
	 * @param provisioningMap
	 *            -a map with all the SecurityProvisioningGroup with the role ID as the key
	 * @return -the string of role ID's
	 */
