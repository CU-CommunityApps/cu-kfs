...
public class VendorDaoImpl implements VendorDao, CuVendorDao {
    ...
    private static final String PARENT_SQL_JOIN =
            ...
            + "LEFT JOIN ("
            + "SELECT LISTAGG(DISTINCT VNDR_ALIAS_NM, ', ') WITHIN GROUP (ORDER BY VNDR_ALIAS_NM ASC) AS ALL_ALIASES, "
            + "VNDR_HDR_GNRTD_ID, VNDR_DTL_ASND_ID "
            + "FROM PUR_VNDR_ALIAS_T "
            + "WHERE DOBJ_MAINT_CD_ACTV_IND = 'Y' "
            + "GROUP BY VNDR_HDR_GNRTD_ID, VNDR_DTL_ASND_ID"
            + ") ALIASES ON DETAIL.VNDR_HDR_GNRTD_ID = ALIASES.VNDR_HDR_GNRTD_ID "
            + "AND DETAIL.VNDR_DTL_ASND_ID = ALIASES.VNDR_DTL_ASND_ID "
