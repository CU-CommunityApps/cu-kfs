...
import edu.cornell.kfs.gl.batch.service.impl.CuIndirectCostRecoveryAccountDistributionMetadata;
...
public class PosterServiceImpl implements PosterService {
     ...
     * set up for ICR
     * 
     * KFSPTS-4390: Needed to use a CU customized metadata class. This also adds the throws declaration to this method.
     *
     * @param et
     * @param reportErrors
     * @return null if the ET does not have a SubAccount properly set up for ICR
     * @throws IndirectCostRecoveryMetadataSearchException 
     */
    protected IndirectCostRecoveryGenerationMetadata retrieveSubAccountIndirectCostRecoveryMetadata(ExpenditureTransaction et) throws IndirectCostRecoveryMetadataSearchException {
        ...
        if (ObjectUtils.isNotNull(subAccount) && ObjectUtils.isNotNull(subAccount.getA21SubAccount())) {
            ...
            if (subAccountOK) {
                ...
                List<CuIndirectCostRecoveryAccountDistributionMetadata> icrAccountList = metadata.getAccountLists();
                for (A21IndirectCostRecoveryAccount a21 : activeICRAccounts){
                    icrAccountList.add(new CuIndirectCostRecoveryAccountDistributionMetadata(a21));
                }
                return metadata;
            }
        }
    ...
    // KFSPTS-4390: Needed to use a CU customized metadata class. This also adds the throws declaration to this method.
    protected IndirectCostRecoveryGenerationMetadata retrieveAccountIndirectCostRecoveryMetadata(ExpenditureTransaction et) throws IndirectCostRecoveryMetadataSearchException {
        ...
        for (IndirectCostRecoveryAccount icr : account.getActiveIndirectCostRecoveryAccounts()){
            icrAccountList.add(new CuIndirectCostRecoveryAccountDistributionMetadata(icr));
        }
