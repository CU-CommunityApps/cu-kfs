...
/**
 * Base service implementation for BalancingService. Useful for generic implementation of common code between labor
 * and GL balancing.
 */
@Transactional
public abstract class BalancingServiceBaseImpl<T extends Entry, S extends Balance> implements BalancingService {
    ...
    public boolean runBalancing() {
        ...
        if (this.getHistoryCount(null, entryHistoryPersistentClass) == 0
                || this.getHistoryCount(null, balanceHistoryPersistentClass) == 0) {
            reportWriterService.writeFormattedMessageLine(kualiConfigurationService
                            .getPropertyValueAsString(KFSKeyConstants.Balancing.MESSAGE_BATCH_BALANCING_DATA_INSERT),
        ...
        if (this.getHistoryCount(obsoleteUniversityFiscalYear, entryHistoryPersistentClass) != 0
                || this.getHistoryCount(obsoleteUniversityFiscalYear, balanceHistoryPersistentClass) != 0
                || this.doesCustomHistoryExist(obsoleteUniversityFiscalYear)) {
        ...
        // We only do update step if history has not been populated. If it has we can't run the update cycle because
        ...
        if (!historyTablesPopulated) {
            ...
            updateRecordsIgnored += this.updateHistoriesHelper(PosterService.MODE_REVERSAL, startUniversityFiscalYear,
                    this.getReversalInputFile(), this.getReversalErrorOutputFile());
            updateRecordsIgnored += this.updateHistoriesHelper(PosterService.MODE_ICR, startUniversityFiscalYear,
                    this.getICRInputFile(), this.getICRErrorOutputFile());
            updateRecordsIgnored += this.updateHistoriesHelper(PosterService.MODE_ICRENCMB, startUniversityFiscalYear,
        ...
        if (countEntryComparisionFailure != 0) {
            ...
            reportWriterService.writeFormattedMessageLine(kualiConfigurationService
                    ...
                    entryHistoryPersistentClass.getSimpleName(), countEntryComparisionFailure,
        ...
        if (countBalanceComparisionFailure != 0) {
            ...
            reportWriterService.writeFormattedMessageLine(kualiConfigurationService
                            .getPropertyValueAsString(KFSKeyConstants.Balancing.MESSAGE_BATCH_BALANCING_FAILURE_COUNT),
                    balanceHistoryPersistentClass.getSimpleName(), countBalanceComparisionFailure,
                    this.getComparisonFailuresToPrintPerReport());
        }
        ...
        if (!historyTablesPopulated) {
            ...
            reportWriterService.writeFormattedMessageLine(
                    kualiConfigurationService.getPropertyValueAsString(
                            KFSKeyConstants.Balancing.MESSAGE_BATCH_BALANCING_FILE_LISTING),
                    this.getFilenames());
        }
        ...
        reportWriterService.writeStatisticLine(kualiConfigurationService
                        .getPropertyValueAsString(KFSKeyConstants.Balancing.REPORT_HISTORY_TABLES_INITIALIZED),
                historyTablesPopulated ? "Yes" : "No");
        reportWriterService.writeStatisticLine(
                kualiConfigurationService.getPropertyValueAsString(KFSKeyConstants.Balancing.REPORT_OBSOLETE_DELETED),
                obsoleteUniversityFiscalYearDeleted ? "Yes (" + obsoleteUniversityFiscalYear + ")" : "No");
        reportWriterService.writeStatisticLine(
                kualiConfigurationService.getPropertyValueAsString(KFSKeyConstants.Balancing.REPORT_UPDATED_SKIPPED),
                updateRecordsIgnored);
        reportWriterService.writeStatisticLine(kualiConfigurationService
                        .getPropertyValueAsString(KFSKeyConstants.Balancing.REPORT_COMPARISION_FAILURE),
                this.getShortTableLabel(entryHistoryPersistentClass.getSimpleName()),
                "(" + entryHistoryPersistentClass.getSimpleName() + ")", countEntryComparisionFailure);
        reportWriterService.writeStatisticLine(kualiConfigurationService
                        .getPropertyValueAsString(KFSKeyConstants.Balancing.REPORT_COMPARISION_FAILURE),
                this.getShortTableLabel(balanceHistoryPersistentClass.getSimpleName()),
                "(" + balanceHistoryPersistentClass.getSimpleName() + ")", countBalanceComparisionFailure);
        reportWriterService.writeStatisticLine(kualiConfigurationService
                        .getPropertyValueAsString(KFSKeyConstants.Balancing.REPORT_ENTRY_SUM_ROW_COUNT_HISTORY),
                this.getShortTableLabel(entryHistoryPersistentClass.getSimpleName()),
                "(" + entryHistoryPersistentClass.getSimpleName() + ")",
                ledgerEntryHistoryBalancingDao.findSumRowCountGreaterOrEqualThan(startUniversityFiscalYear));
        reportWriterService.writeStatisticLine(kualiConfigurationService
                        .getPropertyValueAsString(KFSKeyConstants.Balancing.REPORT_ENTRY_ROW_COUNT_PRODUCTION),
                this.getShortTableLabel(Entry.class.getSimpleName()),
                ledgerEntryBalancingDao.findCountGreaterOrEqualThan(startUniversityFiscalYear));
        reportWriterService.writeStatisticLine(kualiConfigurationService
                        .getPropertyValueAsString(KFSKeyConstants.Balancing.REPORT_BALANCE_ROW_COUNT_HISTORY),
                this.getShortTableLabel(balanceHistoryPersistentClass.getSimpleName()),
                "(" + balanceHistoryPersistentClass.getSimpleName() + ")",
                this.getHistoryCount(null, balanceHistoryPersistentClass));
        reportWriterService.writeStatisticLine(kualiConfigurationService
                        .getPropertyValueAsString(KFSKeyConstants.Balancing.REPORT_BALANCE_ROW_COUNT_PRODUCTION),
                this.getShortTableLabel(Balance.class.getSimpleName()),
                ledgerBalanceBalancingDao.findCountGreaterOrEqualThan(startUniversityFiscalYear));

        if (ObjectUtils.isNotNull(countCustomComparisionFailures)) {
            for (String name : countCustomComparisionFailures.keySet()) {
                int count = countCustomComparisionFailures.get(name);
                reportWriterService.writeStatisticLine(kualiConfigurationService
                                .getPropertyValueAsString(KFSKeyConstants.Balancing.REPORT_COMPARISION_FAILURE),
                        this.getShortTableLabel(name), "(" + name + ")", count);
            }
        }
    ...
    protected boolean isFilesReady() {
        ...
        return inputFile != null && errorFile != null && inputFile.exists() && errorFile.exists()
                && inputFile.canRead() && errorFile.canRead();
    }
     ...
     */
    protected void deleteHistory(Integer universityFiscalYear, Class<? extends PersistableBusinessObject> persistentClass) {
     ...
     * @return count
     */
    protected int getHistoryCount(Integer fiscalYear, Class<? extends PersistableBusinessObject> persistentClass) {
    ...
    protected int updateHistoriesHelper(Integer postMode, Integer startUniversityFiscalYear, File inputFile, File errorFile) {
        ...
        try {
            FileReader posterInputFileReader = new FileReader(inputFile, StandardCharsets.UTF_8);
            BufferedReader posterInputBufferedReader = new BufferedReader(posterInputFileReader);
            FileReader posterErrorFileReader = new FileReader(errorFile, StandardCharsets.UTF_8);
                ...
                if (StringUtils.isNotEmpty(currentInputLine) && StringUtils.isNotBlank(currentInputLine.trim())) {
                    ...
                    if (currentInputLine.equals(currentErrorLine)) {
                        ...
                        if (originEntry.getUniversityFiscalYear() >= startUniversityFiscalYear) {
                            ...
                            ignoredRecordsFound++;
                            reportWriterService.writeError(originEntry, new Message(kualiConfigurationService
            ...
            LOG.fatal(kualiConfigurationService
                            .getPropertyValueAsString(KFSKeyConstants.Balancing.ERROR_BATCH_BALANCING_UNKNOWN_FAILURE),
                    e.getMessage(), lineNumber, e);
            reportWriterService.writeFormattedMessageLine(String.format(kualiConfigurationService
                            .getPropertyValueAsString(KFSKeyConstants.Balancing.ERROR_BATCH_BALANCING_UNKNOWN_FAILURE),
                    e.getMessage(), lineNumber));
            throw new RuntimeException(String.format(kualiConfigurationService
                            .getPropertyValueAsString(KFSKeyConstants.Balancing.ERROR_BATCH_BALANCING_UNKNOWN_FAILURE),
                    e.getMessage(), lineNumber), e);
        }
    ...
    protected abstract int compareBalanceHistory();

    protected abstract int compareEntryHistory();
