...
import org.kuali.kfs.kim.impl.common.delegate.DelegateMemberAttributeDataBo;
...
public class RoleServiceImpl extends RoleServiceBase implements RoleService {
    ...
    public RoleMemberQueryResults findRoleMembers(QueryByCriteria queryByCriteria) throws RiceIllegalStateException {
        ...
        GenericQueryResults<RoleMemberBo> results = getCriteriaLookupService().lookup(RoleMemberBo.class,
                queryByCriteria, lc.build());

        RoleMemberQueryResults.Builder builder = RoleMemberQueryResults.Builder.create();
        builder.setMoreResultsAvailable(results.isMoreResultsAvailable());
        builder.setTotalRowCount(results.getTotalRowCount());

        final List<RoleMember.Builder> ims = new ArrayList<>();
    ...
    public List<RoleResponsibilityAction> getRoleMemberResponsibilityActions(String roleMemberId) throws
        ...
        List<RoleResponsibilityAction> roleResponsibilityActionsList = new ArrayList<>();
    ...
    @Override
    public DelegateMemberQueryResults findDelegateMembers(QueryByCriteria queryByCriteria) throws
            RiceIllegalStateException {
        incomingParamCheck(queryByCriteria, "queryByCriteria");
        ...
        GenericQueryResults<DelegateMemberBo> results = getCriteriaLookupService().lookup(DelegateMemberBo.class,
                queryByCriteria, lc.build());
        ...
        final List<DelegateMember.Builder> ims = new ArrayList<>();
    ...
    protected List<Role> loadRoles(List<String> roleIds) {
        ...
        Map<String, Role> roleMap = new HashMap<>(roleIds.size());
        for (String roleId : roleIds) {
            Role role = getRoleFromCache(roleId);
            if (role != null) {
    ...
    public List<Map<String, String>> getRoleQualifersForPrincipalByRoleIds(String principalId, List<String> roleIds,
            ...
            if (MemberType.PRINCIPAL.equals(roleMemberBo.getType())) {
                ...
                if (roleTypeService != null) {
                    List<RoleMembership> las =
                            roleIdToMembershipMap.computeIfAbsent(roleMemberBo.getRoleId(), k -> new ArrayList<>());
                    RoleMembership mi = RoleMembership.Builder.create(roleMemberBo.getRoleId(), roleMemberBo.getId(),
                            roleMemberBo.getMemberId(), roleMemberBo.getType(), roleMemberBo.getAttributes()).build();
        ...
        for (Map.Entry<String, List<RoleMembership>> entry : roleIdToMembershipMap.entrySet()) {
            ...
            // it is possible that the the roleTypeService is coming from a remote application and therefore it can't
            // be guaranteed that it is up and working, so using a try/catch to catch this possibility.
            try {
            ...
            if (MemberType.PRINCIPAL.equals(roleMemberBo.getType())
                ...
                if (roleTypeService != null) {
                    ...
                    try {
                        ...
                        LOG.warn("Not able to retrieve RoleTypeService from remote system for roleBo Id: " +
                                roleBo.getId(), ex);
                    }
                }
    ...
    public RoleQueryResults findRoles(QueryByCriteria queryByCriteria) throws RiceIllegalStateException {
        ...
        final List<Role.Builder> ims = new ArrayList<>();
    ...
    @Override
    public RoleMembershipQueryResults findRoleMemberships(QueryByCriteria queryByCriteria) throws
            RiceIllegalStateException {
        incomingParamCheck(queryByCriteria, "queryByCriteria");
        ...
        final List<RoleMembership.Builder> ims = new ArrayList<>();
        for (RoleMemberBo bo : results.getResults()) {
    ...
    @Override
    public List<DelegateMember> getDelegationMembersByDelegationId(String delegationId) throws
            RiceIllegalStateException {
    ...
    @Override
    public DelegateMember getDelegationMemberByDelegationAndMemberId(String delegationId, String memberId) throws
            RiceIllegalStateException {
        incomingParamCheck(delegationId, "delegationId");
        incomingParamCheck(memberId, "memberId");
    ...
    public List<RoleResponsibility> getRoleResponsibilities(String roleId) throws RiceIllegalStateException {
        ...
        List<RoleResponsibility> roleResponsibilities = new ArrayList<>();
                ...
                if (MemberType.ROLE.equals(roleMemberBo.getType())) {
                    ...
                    if (roleTypeService != null) {
                        ...
                        nestedRoleQualification = roleTypeService.convertQualificationForMemberRoles(
                                ...
                                roles.get(roleMemberBo.getRoleId()).getName(), memberRole.getNamespaceCode(),
    ...
    protected void linkDelegateToRoleMembership(DelegateTypeBo delegation,
            DelegateMember.Builder delegateMemberBuilder, RoleMembership.Builder roleMembershipBuilder) {
    ...
    protected void resolveDelegationMemberRoles(List<RoleMembership.Builder> membershipBuilders,
        ...
        for (RoleMembership.Builder roleMembership : membershipBuilders) {
            ...
            for (DelegateType.Builder delegation : roleMembership.getDelegates()) {
                List<DelegateMember.Builder> newMembers = new ArrayList<>();
        ...
        try {
            ...
            for (Role role : roles) {
                ...
                if (!rolesCheckedForExactMatch.contains(role.getId())) {
                    List<RoleMemberBo> matchingPrincipalRoleMembers = getRoleMembersForPrincipalId(role.getId(),
                            principalId);
            ...
            List<RoleMemberBo> roleMemberBos = getStoredRoleMembersForRoleIds(new ArrayList<>(roleIndex.keySet()),
                    ...
                    try {
                        if (roleTypeService.doesRoleQualifierMatchQualification(qualification,
                            ...
                            if (principalHasRole(context, principalId,
                                    Collections.singletonList(roleMemberBo.getMemberId()), nestedRoleQualification,
                                    true)) {
                                return putPrincipalHasRoleInCache(true, principalId, role.getId(), qualification,
                                        checkDelegations);
                            }
    ...
    protected boolean putPrincipalHasRoleInCache(boolean principalHasRole, String principalId, String roleId,
            Map<String, String> qualification, boolean checkDelegations) {
        String key = buildPrincipalHasRoleCacheKey(principalId, roleId, qualification, checkDelegations);
        cacheManager.getCache(Role.Cache.NAME).put(key, principalHasRole);
     ...
     * original role (like a RolePrincipal or RoleGroup.) And then, the members of that delegateBo may have additional
     ...
     * that org. But, then they delegate authority for that organization (not their authority - the delegateBo is
     * attached to the org.) So, in this case the delegateBo has a qualifier of the organization when it is attached
     * to the role.
     * <p>
    ...
    protected boolean matchesOnDelegation(Set<String> allRoleIds, String principalId, List<String> principalGroupIds,
        ...
        Map<String, List<DelegateTypeBo>> roleToDelegations = new HashMap<>();
        for (DelegateTypeBo delegation : delegations.values()) {
            List<DelegateTypeBo> roleDelegations =
                ...
                for (DelegateMemberBo delegateMemberBo : delegation.getMembers()) {
                    ...
                    if (MemberType.ROLE.equals(delegateMemberBo.getType())
                            && !principalHasRole(principalId, Collections.singletonList(
                                    delegateMemberBo.getMemberId()), qualification, false)) {
                        continue;
                    }
                    ...
                    try {
                        ...
                        LOG.warn("Unable to call doesRoleQualifierMatchQualification on role type service for " +
                                ...
                                delegateMemberBo.getQualifier(), ex);
                        continue;
                    }
                    ...
                    if (StringUtils.isNotBlank(delegateMemberBo.getRoleMemberId())) {
                        ...
                        if (rm != null) {
                            ...
                            // delegateBo from surviving the inactivation/ changing of the main person's role
                            // membership
    ...
    /**
     * @return a KimDelegationImpl object by its ID. If the delegateBo already exists in the cache, this method will
    ...
    /**
     * @return a KimDelegationMemberImpl object by its ID and the ID of the delegation it belongs to. If the
     *         delegation member exists in the cache, this method will return the cached one; otherwise, it will
     *         retrieve the uncached version from the database and then cache it before returning it.
     */
    ...
    private List<RoleMemberBo> getStoredRoleMembersUsingExactMatchOnQualification(String principalId,
        ...
        List<RoleMemberBo> roleMemberBoList = new ArrayList<>();
        ...
        for (String roleId : roleIds) {
            ...
            if (roleTypeService != null) {
                try {
                    ...
                    if (CollectionUtils.isNotEmpty(attributesForExactMatch)) {
                        copyRoleIds.remove(roleId);
                        roleMemberBoList.addAll(getStoredRoleMembersForRoleIdsWithFilters(
    ...
    private List<RoleMemberBo> getStoredRoleGroupsUsingExactMatchOnQualification(List<String> groupIds,
        ...
        List<String> copyRoleIds = new ArrayList<>(roleIds);
        List<RoleMemberBo> roleMemberBos = new ArrayList<>();
    ...
    private List<DelegateMember> getDelegateMembersForDelegation(DelegateTypeBo delegateBo) {
        ...
        List<DelegateMember> delegateMembersReturnList = new ArrayList<>();
    ...
    public RoleMember createRoleMember(RoleMember roleMember) throws RiceIllegalStateException {
        ...
        List<RoleMemberAttributeDataBo> attrBos = KimAttributeDataBo.createFrom(RoleMemberAttributeDataBo.class,
    ...
    @Override
    public RoleMember updateRoleMember(@WebParam(name = "roleMember") RoleMember roleMember) throws
        ...
        RoleMemberBo bo = RoleMemberBo.from(roleMember);
        List<RoleMemberAttributeDataBo> updateAttrBos = new ArrayList<>();
    ...
    @Override
    public DelegateMember updateDelegateMember(@WebParam(name = "delegateMember") DelegateMember delegateMember)
        ...
        List<DelegateMemberAttributeDataBo> attrBos = KimAttributeDataBo.createFrom(
        ...
        List<DelegateMemberAttributeDataBo> updateAttrBos = new ArrayList<>();
        ...
        if (originalDelegateMemberBo != null) {
            ...
            for (DelegateMemberAttributeDataBo newDelegateMemberAttrDataBo : attrBos) {
                for (DelegateMemberAttributeDataBo oldDelegateMemberAttrDataBo :
                        originalDelegateMemberBo.getAttributeDetails()) {
                    if (newDelegateMemberAttrDataBo.getKimTypeId().equals(oldDelegateMemberAttrDataBo.getKimTypeId())
                            && newDelegateMemberAttrDataBo.getKimAttributeId().equals(
                                    oldDelegateMemberAttrDataBo.getKimAttributeId())) {
    ...
    @Override
    public DelegateMember createDelegateMember(@WebParam(name = "delegateMember") DelegateMember delegateMember)
            throws RiceIllegalArgumentException, RiceIllegalStateException {
        ...
        if (delegateMember.getDelegationMemberId() != null) {
            throw new RiceIllegalStateException("the delegate member already exists: " +
    ...
    @Override
    public void removeDelegateMembers(@WebParam(name = "delegateMembers") List<DelegateMember> delegateMembers)
            throws RiceIllegalArgumentException, RiceIllegalStateException {
    ...
    public RoleResponsibilityAction createRoleResponsibilityAction(RoleResponsibilityAction roleResponsibilityAction)
        ...
        if (StringUtils.isNotBlank(roleResponsibilityAction.getId())
            ...
            throw new RiceIllegalStateException("the roleResponsibilityAction to create already exists: " +
    ...
    public RoleResponsibilityAction updateRoleResponsibilityAction(RoleResponsibilityAction roleResponsibilityAction)
        ...
        if (StringUtils.isBlank(roleResponsibilityAction.getId())
                || getRoleResponsibilityActionBo(roleResponsibilityAction.getId()) == null) {
            throw new RiceIllegalStateException("the roleResponsibilityAction to create does not exist: " +
                    roleResponsibilityAction);
        }
    ...
    public void deleteRoleResponsibilityAction(String roleResponsibilityActionId)
        ...
        if (StringUtils.isBlank(roleResponsibilityActionId) || bo == null) {
            throw new RiceIllegalStateException("the roleResponsibilityAction to delete does not exist: " +
    ...
    @Override
    public DelegateType createDelegateType(DelegateType delegateType) throws RiceIllegalArgumentException,
            RiceIllegalStateException {
    ...
    @Override
    public DelegateType updateDelegateType(DelegateType delegateType) throws RiceIllegalArgumentException,
            RiceIllegalStateException {
        incomingParamCheck(delegateType, "delegateType");
    ...
    public void assignPermissionToRole(String permissionId, String roleId) throws RiceIllegalArgumentException {
        ...
        Long nextSeq = KRADServiceLocator.getSequenceAccessorService().getNextAvailableSequenceNumber(
                KimConstants.SequenceNames.KRIM_ROLE_PERM_ID_S, RolePermissionBo.class);
    ...
    public void revokePermissionFromRole(String permissionId, String roleId) throws RiceIllegalArgumentException {
        ...
        Collection<RolePermissionBo> rolePermissionBos = getBusinessObjectService()
                .findMatching(RolePermissionBo.class, params);
        List<RolePermissionBo> rolePermsToSave = new ArrayList<>();
    ...
    protected void addMemberAttributeData(RoleMemberBo roleMember, Map<String, String> qualifier, String kimTypeId) {
        List<RoleMemberAttributeDataBo> attributes = new ArrayList<>();
        for (Map.Entry<String, String> entry : qualifier.entrySet()) {
            ...
            List<RoleMemberAttributeDataBo> origRoleMemberAttributes =
                    (List<RoleMemberAttributeDataBo>) getBusinessObjectService()
                            .findMatching(RoleMemberAttributeDataBo.class, criteria);
    ...
    protected void addDelegationMemberAttributeData(DelegateMemberBo delegationMember, Map<String, String> qualifier,
            String kimTypeId) {
        List<DelegateMemberAttributeDataBo> attributes = new ArrayList<>();
        for (Map.Entry<String, String> entry : qualifier.entrySet()) {
            ...
            List<DelegateMemberAttributeDataBo> origDelegationMemberAttributes =
                    (List<DelegateMemberAttributeDataBo>) getBusinessObjectService()
                            .findMatching(DelegateMemberAttributeDataBo.class, criteria);
            DelegateMemberAttributeDataBo origDelegationMemberAttribute =
                    (origDelegationMemberAttributes != null && !origDelegationMemberAttributes.isEmpty())
                            ? origDelegationMemberAttributes.get(0) : null;
    ...
    private void deleteNullMemberAttributeData(List<RoleMemberAttributeDataBo> attributes) {
        List<RoleMemberAttributeDataBo> attributesToDelete = new ArrayList<>();
    ...
    private void deleteNullDelegationMemberAttributeData(List<DelegateMemberAttributeDataBo> attributes) {
        List<DelegateMemberAttributeDataBo> attributesToDelete = new ArrayList<>();
