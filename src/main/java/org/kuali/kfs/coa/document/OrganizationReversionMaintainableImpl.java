...
public class OrganizationReversionMaintainableImpl extends FinancialSystemMaintainable {
    ...
    public void setBusinessObject(PersistableBusinessObject businessObject) {
        ...
        List<OrganizationReversionDetail> details = organizationReversion.getOrganizationReversionDetails();

        if (details == null) {
            details = new ArrayList<OrganizationReversionDetail>();
            organizationReversion.setOrganizationReversionDetails(details);
        }

        if (details.size() == 0) {

            Collection<ReversionCategory> categories = organizationReversionService.getCategoryList();

            for (ReversionCategory category : categories) {
                if (category.isActive()) {
                    OrganizationReversionDetail detail = new OrganizationReversionDetail();
                    detail.setOrganizationReversionCategoryCode(category.getReversionCategoryCode());
    ...
    protected boolean isRelationshipRefreshable(Class boClass, String relationshipName) {
        if (relationshipName.equals("organizationReversionDetails")) {
            return false;
        }
        else {
        ...
        if (field.getContainerRows() != null) {
            for (Row containerRow : field.getContainerRows()) {
                for (Field containedField : containerRow.getFields()) {
                    if (containedField.getPropertyName().matches("organizationReversionDetails\\[\\d+\\]\\.reversionCategory\\.reversionCategoryName")) {
