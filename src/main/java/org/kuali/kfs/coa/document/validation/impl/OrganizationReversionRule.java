...
public class OrganizationReversionRule extends MaintenanceDocumentRuleBase {
    ...
    protected boolean validateDetailBusinessObjects(OrganizationReversion orgReversion) {
        ...
        List<OrganizationReversionDetail> details = orgReversion.getOrganizationReversionDetails();
        int index = 0;
        int originalErrorCount = GlobalVariables.getMessageMap().getErrorCount();
        for (OrganizationReversionDetail dtl : details) {
            String errorPath = "organizationReversionDetails[" + index + "]";
    ...
    protected boolean validateOrganizationReversionDetail(OrganizationReversionDetail detail) {
        ...
        LOG.debug("organization reversion finanical object = " + detail.getReversionObject().getName());
        if (ObjectUtils.isNull(detail.getReversionObject())) {
            result = false;
            GlobalVariables.getMessageMap().putError("organizationReversionObjectCode", KFSKeyConstants.ERROR_EXISTENCE, new String[] { "Financial Object Code: " + detail.getReversionObjectCode() });
        }
        return result;
    }
