...
public class AccountDelegateMaintainableImpl extends FinancialSystemMaintainable {
    ...
    /**
     * This method sets the start date on {@link AccountDelegate} BO
     */
    protected void setStartDateDefault() {
        if (this.businessObject instanceof AccountDelegate) {
            ...
            delegate.setAccountDelegateStartDate(new Date(SpringContext.getBean(DateTimeService.class)
                    .getCurrentDate().getTime()));
        }
    }

    /**
     * Generates the appropriate maintenance locks for the {@link AccountDelegate}
     */
    @Override
    public List<MaintenanceLock> generateMaintenanceLocks() {
        ...
        if (delegate.isAccountsDelegatePrmrtIndicator()) {
            locks.add(createMaintenanceLock(new String[]{
                "chartOfAccountsCode",
                "accountNumber",
    ...
    /**
     * This method returns the field value of a given field, converting the value to a String and encrypting it if
     * necessary
     *
     * @param fieldName
     * @param ddService
    ...
    protected String retrieveFieldValueForLock(String fieldName, DataDictionaryService ddService,
        ...
        if (SpringContext.getBean(BusinessObjectAuthorizationService.class)
                .attributeValueNeedsToBeEncryptedOnFormsAndLinks(getDataObjectClass(), fieldName)) {
            try {
    ...
    public void saveBusinessObject() {
        ...
        accountDelegateService.updateDelegationRole(
                getDocumentNumber(),
                Set.of(accountDelegate.getAccountNumber()),
                Set.of(accountDelegate.getFinancialDocumentTypeCode())
        );
    }
