...
abstract public class IndirectCostRecoveryAccountsRule extends KfsMaintenanceDocumentRuleBase {
    ...
    protected boolean checkIndirectCostRecoveryAccount(String chartOfAccountsCode, String accountNumber, BigDecimal icraAccountLinePercentage, boolean active) {
        boolean success = true;
        ...
        if (StringUtils.isNotBlank(chartOfAccountsCode) && StringUtils.isNotBlank(accountNumber)) {
			...
			// KFSPTS-4566 check if account is closed
			if (success && active) {
				Collection<Account> accounts = SpringContext.getBean(BusinessObjectService.class).findMatching(Account.class, chartAccountMap);
				if (ObjectUtils.isNotNull(accounts) && accounts.size() > 0) {
					Account icrAccount = accounts.iterator().next();
					if (icrAccount.isClosed()) {
	...
	public void setIndirectCostRecoveryAccountList(
			List<? extends IndirectCostRecoveryAccount> indirectCostRecoveryAccountList) {
