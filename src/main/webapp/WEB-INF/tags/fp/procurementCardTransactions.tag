...
<kul:tab tabTitle="Accounting Lines" defaultOpen="true" tabErrorKey="${KFSConstants.TARGET_ACCOUNTING_LINE_ERROR_PATTERN},document.transactionEntries*">
  ...
  <logic:iterate indexId="ctr" name="KualiForm" property="document.transactionEntries" id="currentTransaction">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="datatable">
	 ...
	 KFSPTS-3478 The original implementation is just the table commented out above.  For accounting lines to tab through the fields correctly,
		the sys-java:accountingLineGroup needs to be in a TAB tag, and in sys-java:accountingLines.  The critical difference seems to be that the image
		tags have a tabIndex attribute that is very high and the actual fields have no tabIndex attribute in this configuration.  In the original configuration
		both the fields and images have no tabIndex attribute.--%>
