...
<c:set var="roleQualificationReadOnly" value="${!securityRequestRole.allowKSRToManageQualifications}"/>
<c:if test="${readOnly}">
  <c:set var="securityRequestRoleQualificationHdr" value="${securityRequestRole.requestRoleQualifications[0]}" />
  <c:set var="roleQualificationReadOnly" value="${readOnly}"/>
</c:if>
...
<table border="0" cellpadding="0" cellspacing="0" class="celltable">
    ...
    <c:if test="${!readOnly}">
        <tr>
            ...
            <c:forEach var="wrappedQualificationDetail" items="${securityRequestRoleQualificationHdr.wrappedRoleQualificationDetails}" varStatus="status">
              <td>
                ...
                <kul:htmlControlAttribute attributeEntry="${attributeEntry}"
                    property="document.securityRequestRoles[${securityRequestRoleIndex}].newRequestRoleQualification.roleQualificationDetails[${status.count-1}].attributeValue" readOnly="${roleQualificationReadOnly}" />
                
                <c:set var="attributeDefinition" value="${qualificationDetail.attributeDefinition}" />    
                <c:if test="${(!empty attributeDefinition.quickFinder) && !roleQualificationReadOnly}"> 
            ...
            <td>
                <c:if test="${!roleQualificationReadOnly}">
        ...
        <tr>
            ...
            <c:forEach var="wrappedQualificationDetail" items="${securityRequestRoleQualification.wrappedRoleQualificationDetails}" varStatus="dtlStatus">
              <td>
                ...
                <c:set var="attributeEntry" value="${qualificationDetail.attributeEntry}" />
                <kul:htmlControlAttribute attributeEntry="${attributeEntry}" 
                    property="document.securityRequestRoles[${securityRequestRoleIndex}].requestRoleQualifications[${qualStatus.count-1}].roleQualificationDetails[${detailIndex}].attributeValue" readOnly="${roleQualificationReadOnly}" />
