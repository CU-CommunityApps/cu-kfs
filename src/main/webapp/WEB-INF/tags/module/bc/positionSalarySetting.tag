...
<div class="tab-container" align="center">
	<c:if test="${not readOnly && budgetConstructionPosition.effective && budgetConstructionPosition.budgetedPosition}">   
		<kul:subtab lookedUpCollectionName="fundingLine" width="${tableWidth}" subTabTitle="Add Funding">      
			<bc:appointmentFundingLineForPosition fundingLine="${KualiForm.newBCAFLine}" fundingLineName="newBCAFLine" hasBeenAdded="false" isKeyFieldsLocked="${isKeyFieldsLocked}" countOfMajorColumns="10">
    ...
    <c:if test="${!fundingLine.purged}">	 
		...
		<c:set var="canUnflag" value="${editable && not isNewLine  && (fundingLine.effectiveCSFTracker != null) &&  not ((fundingLine.effectiveCSFTracker.csfFundingStatusCode eq BCConstants.ACTIVE_CSF_DELETE_CODE) || (fundingLine.effectiveCSFTracker.csfFundingStatusCode eq KFSConstants.EMPTY_STRING))}" />
	    ...
	    <kul:subtab lookedUpCollectionName="fundingLine" width="${tableWidth}" subTabTitle="${fundingLine.appointmentFundingString}" >
	    	<bc:appointmentFundingLineForPosition fundingLine="${fundingLine}" fundingLineName="${fundingLineName}"	
	    		countOfMajorColumns="10" lineIndex="${status.index}" hasBeenAdded = "true" readOnly="${readOnly}">   
	    		<c:if test="${canUnflag}">
	    		<!--Unflag button -->
