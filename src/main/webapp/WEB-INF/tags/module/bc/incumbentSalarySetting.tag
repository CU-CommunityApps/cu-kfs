...
<div class="tab-container" align=center>
	<c:if test="${not readOnly}">
		<kul:subtab lookedUpCollectionName="fundingLine" width="${tableWidth}" subTabTitle="Add Funding">      
			<bc:appointmentFundingLineForIncumbent fundingLine="${KualiForm.newBCAFLine}" fundingLineName="newBCAFLine" countOfMajorColumns="13" isKeyFieldsLocked="${isKeyFieldsLocked}" hasBeenAdded="false">
	...
	<c:if test="${!fundingLine.purged}">	
		...
		<c:set var="canUnflag" value="${editable && not isNewLine && not (fundingLine.effectiveCSFTracker.csfFundingStatusCode eq BCConstants.ACTIVE_CSF_DELETE_CODE)}" />
	...
	<div class="tab-container" align="center">        		
	    <c:forEach items="${KualiForm.budgetConstructionIntendedIncumbent.pendingBudgetConstructionAppointmentFunding}" var="fundingLine" varStatus="status">
			<c:if test="${fundingLine.purged}">	
			    ...
			    <kul:subtab lookedUpCollectionName="fundingLine" width="${tableWidth}" subTabTitle="${fundingLine.appointmentFundingString}">
			    	<bc:appointmentFundingLineForIncumbent fundingLine="${fundingLine}" fundingLineName="${fundingLineName}"	countOfMajorColumns="10" lineIndex="${status.index}" readOnly="true" hasBeenAdded = "true">    		
