...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="datatable" id="${id}">
	<c:forEach var="federalFunding" items="${federalFundingIndicators}" varStatus="indicatorStatus">
		...
		<c:if test="${fn:length(subtotalGroup) > 0}" >
			<tr>
				<td colspan="${colspanOfTotalLabel}" class="infoline"><div class="right"><strong>Subtotal:</strong></div></td>
	...
	<c:if test="${fn:length(grandTotalFieldNames) > 0}" >
		...
		<tr>				
			<td colspan="${colspanOfTotalLabel}" class="infoline"><div class="right"><strong>Grand Total:</strong></div></td>
			<c:set var="percentageRed" 		value	='false'/>
			<c:set var="totalRed" 		value	='false'/>

						
			<c:if test="${KualiForm.document['totalEffortPercent'] !='100' }">
				<c:set var="percentageRed" 		value	='true'/>
			</c:if>
				
			<c:if test="${KualiForm.document['totalPayrollAmount'] != KualiForm.document['totalOriginalPayrollAmount']}">
				<c:set var="totalRed" 		value	='true'/>
			</c:if>
			
	 		<ec:detailLineTotal percentageRed = "${percentageRed}"  totalRed = "${totalRed}"  totalFieldNames="${grandTotalFieldNames}" readOnlySection="${readOnlySection}" hasActions="${hasActions}" />	
		</tr>
		
		<c:choose>
			<c:when test="${colspanOfTotalLabel == '4'}">
				<c:set var="emptyColumnSpan" 		value	='5'/>
				<c:set var="percentageMethod2call" 	value	='methodToCall.reviewPercentagesSummary'/>
				<c:set var="totalMethod2call" 		value	='methodToCall.reviewTotalsSummary'/>
			</c:when>
			<c:otherwise>
				<c:set var="emptyColumnSpan" 		value	='9'/>
				<c:set var="percentageMethod2call" 	value	='methodToCall.reviewPercentagesDetail'/>
				<c:set var="totalMethod2call" 		value	='methodToCall.reviewTotalsDetail'/>
			</c:otherwise>		
		</c:choose>
