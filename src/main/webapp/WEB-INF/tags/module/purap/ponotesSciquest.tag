...
<c:if test="${not empty attachmentTypesValuesFinderClass}">
  <c:set var="noteColSpan" value="${noteColSpan + 2}" />
</c:if>
...
<%-- kfspts-794 --%>
            <c:set var="finderClass1"
                   value="${fn:replace(sendToVendorValuesFinderClass,'.','|')}"/>
            <td class="infoline">
                <div align="center">
                  <c:choose>
                    ...
                    <html:select property="newNote.noteTopicText">
                     ...
                     <td class="infoline"><div align="center"><html:image property="methodToCall.insertBONote" onclick="${noteAddButtonOnClick}" src="${ConfigProperties.kr.externalizable.images.url}tinybutton-add1.gif" alt="Add a Note" title="Add a Note" styleClass="tinybutton"/></div></td>	 
			       </tr>
			   </c:if>   
...
<%-- NEED TO ADD THIS TOPIC FIELD TO DATABASE --%>
                        ...
                        <td class="datacell center"><bean:write name="KualiForm" property="document.notes[${status.index}].noteText"/></td>
