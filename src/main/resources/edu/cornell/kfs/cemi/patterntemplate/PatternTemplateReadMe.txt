    ...
    ==============================================================================================================
     ...
     (9) There are times when validation runs of the data extraction batch jobs need to be performed in the actual CEMI
         server environment. We do not want to retain the data generated by those executions. There is a SQL file in the
         nonprod-sql repository manual folder containing the boiler plate scripts for that test run data cleanup.
         
         Each table requiring test run cleanup should have two SQL statements present in that file for the data 
         extraction being created. Make those entries per the instructions in file:
                manual / kfs / KFSPTS-38161-deleteCemiTestRunData.sql 
